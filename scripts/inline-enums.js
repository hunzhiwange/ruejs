...
// https://github.com/vuejs/core/blob/main/scripts/inline-enums.js
...
export function scanEnums() {
  ...
  for (const relativeFile of files) {
    ...
    // Only process TypeScript source files; skip directories and other file types
      ...
      if (
        ...
        for (let i = 0; i < decl.members.length; i++) {
          ...
          const saveValue = (/** @type {string | number} */ value) => {
            ...
            // For example: enum ErrorCodes exist in both @vue/compiler-core and @rue-js/runtime
            ...
            // e.g. 1 << 2
              ...
              const resolveValue = (
                ...
                if (node.type === 'NumericLiteral' || node.type === 'StringLiteral') {
                  ...
                  if (!(exp in defines)) {
                    throw new Error(`unhandled enum initialization expression ${exp} in ${file}`)
                  }
                  return defines[exp]
                } else {
                  throw new Error(`unhandled BinaryExpression operand type ${node.type} in ${file}`)
                }
              }
              const exp = `${resolveValue(init.left)}${init.operator}${resolveValue(init.right)}`
              value = evaluate(exp)
