...
const ApiDocDetail: FC = () => {
  ...
  watch(
    ...
    async (data: any) => {
      ...
      if (!seg) return
      setDocPath(seg)
      const docsMeta = SECTIONS_BY_TYPE['api'].flatMap(sec => flatten(sec.items))
    ...
    <SidebarPlayground>
      ...
      {currentIndex.get() >= 0 && (
        <div className="mt-8 flex justify-between">
          ...
          {next.get() ? (
            <RouterLink to={`${uiBase.value}/${next.get()?.id}`} className="btn btn-outline btn-sm">
              下一页：{next?.get()?.title} →
            </RouterLink>
          ) : (
            <span />
