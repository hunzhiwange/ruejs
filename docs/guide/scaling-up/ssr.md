...
4. 运行 `npm install rue-js rue-server`
5. 创建一个 `example.ts` 文件：

```ts
// 这在 Node.js 服务器上运行
import { createSSRApp, h } from 'rue-js'
...
3. 通过添加 [Import Map](https://github.com/WICG/import-maps) 到 HTML shell 来支持在浏览器中使用 `import * from 'rue-js'` 这样的用法。
