...
:::danger 已移除的实验性功能
响应式转换是一个实验性功能，已在最新的 3.4 版本中移除。请阅读[这里的原因](https://github.com/rue-jsjs/rfcs/discussions/369#discussioncomment-5059028)。
...
[Rue 响应式转换](https://github.com/rue-jsjs/core/tree/main/packages/reactivity-transform)是一个编译时转换，允许我们编写这样的代码：
...
这些宏是全局可用的，在启用响应式转换时不需要导入，但如果你想更明确，可以从 `rue-js/macros` 选择性地导入它们：

```js
import { $ref } from 'rue-js/macros'
...
```ts
/// <reference types="rue-js/macros-global" />
```

当从 `rue-js/macros` 显式导入宏时，类型将无需声明全局变量即可工作。
