...
```tsx
import { useRef, useEffect } from 'rue-js'
...
当用户更改路由时，将焦点返回到页面的最开始，就在跳过链接之前。这可以通过在 `backToTop` ref 上调用 focus 来实现（假设使用了 `rue-js-router`）：

```tsx
import { useEffect, useRef } from 'rue-js'
import { useLocation } from 'rue-js-router'
import type { FC } from 'rue-js'
