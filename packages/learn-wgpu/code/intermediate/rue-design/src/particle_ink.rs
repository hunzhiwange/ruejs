...
impl ParticleInk {
    pub fn new(
        ...
        // 粒子像素尺寸（根据硬件限制自适应缩放，避免 dispatch 超限）
        let psize_base = app.scale_factor * 1.0;
        let total_pixels = (w as f64) * (h as f64);
        let max_particles = (65535f64) * 64f64;
        let required_min_psize = ((total_pixels / max_particles).sqrt()) as f32;
        let particle_point_size = if psize_base < required_min_psize {
            required_min_psize
        } else {
            psize_base
        };
        ...
        // 准备绑定组需要的数据
        let total = (particle_num.width as u64) * (particle_num.height as u64);
        let groups = ((total + 63) / 64) as u32;
        let groups_clamped = core::cmp::min(groups, 65535);
        let bind_group_data = BindGroupData {
            uniforms: vec![&particle_uniform_buf],
            storage_buffers: vec![&particle_buffer],
            visibilitys: vec![wgpu::ShaderStages::COMPUTE],
            workgroup_count: (groups_clamped, 1, 1),
            ..Default::default()
        };
