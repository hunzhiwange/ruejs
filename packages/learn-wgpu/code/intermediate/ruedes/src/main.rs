...
pub fn main() -> iced::Result {
    ...
    #[cfg(target_arch = "wasm32")]
    {
        ...
        window.platform_specific.target = Some("ruedes-wasm-app-container".to_string());
...
impl Tour {
    ...
    fn welcome(&self) -> Column<'_, Message> {
        Self::container("Welcome to WebGPU!")
            .push(
...
fn ferris<'a>(width: u32, filter_method: image::FilterMethod) -> Container<'a, Message> {
    center_x(
        image(iced::widget::image::Handle::from_bytes(
            include_bytes!("../images/ferris.png").as_slice(),
        ))
