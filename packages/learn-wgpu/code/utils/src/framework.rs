...
#[cfg(target_arch = "wasm32")]
pub fn attach_wgpu_canvas_with(window: &Window, target_id: &str) {
    let canvas = window.canvas().unwrap();
    web_sys::window()
        .and_then(|win| win.document())
        .map(|doc| {
            let _ = canvas.set_attribute("id", "winit-canvas");
            if let Some(dst) = doc.get_element_by_id(target_id) {
                if let Some(parent) = dst.parent_node() {
                    let new_node: web_sys::Node = canvas.clone().unchecked_into();
                    let old_node: web_sys::Node = dst.unchecked_into();
                    let _ = parent.replace_child(&new_node, &old_node);
                }
            }
        })
        .expect("Unable to attach canvas to the current document");
    canvas.set_tab_index(0);
    let style = canvas.style();
    style.set_property("outline", "none").unwrap();
    canvas.focus().expect("Canvas failed to gain focus");
}
