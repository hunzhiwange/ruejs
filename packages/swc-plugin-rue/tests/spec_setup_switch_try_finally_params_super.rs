//! SWC 插件转换行为测试（spec_setup_switch_try_finally_params_super）
//!
//! 覆盖：此用例的转换快照对比。
  ...
  switch (a.value) {
    case 0: {
      try {
        ...
        onBeforeUnmount(() => {
          watchEffect(() => console.log(`fin=${a.value}-${a.value > 0 ? 'x' : 'y'}`, [a.value, { k: a.value }], { x: a.value, arr: [a.value] }))
        })
      }
      break
    }
    default: {
      watchEffect(() => onBeforeUnmount(() => console.log([a.value, 'k', { t: `v=${a.value}` }], { z: a.value })))
    }
  }
