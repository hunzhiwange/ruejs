//! SWC 插件转换行为测试（spec15）
//!
//! 覆盖：TransitionGroup 列表过渡示例的 JSX → Vapor 转换。
use swc_plugin_rue::apply;

mod utils;

#[test]
fn transforms_spec15() {
    let src = r##"
...
import { type FC, TransitionGroup, ref, _$vaporWithHookId, useSetup, vapor, renderBetween, _$createElement, _$createComment, _$createTextNode, _$settextContent, _$createDocumentFragment, _$appendChild, watchEffect, _$vaporKeyedList, _$createTextWrapper, _$setAttribute, _$addEventListener, _$setClassName } from 'rue-js';
const ListTransitionExample: FC = ()=>{
...
"##;
    ...
    std::fs::create_dir_all("target/vapor_outputs").ok();
    std::fs::write("target/vapor_outputs/spec15.out.js", strip_marker(&out)).ok();
