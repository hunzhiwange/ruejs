//! SWC 插件转换行为测试（spec_untyped_arrow）
//!
//! 覆盖：此用例的转换快照对比。
...
#[test]
fn transforms_untyped_arrow_component_setup() {
    let src = r##"
...
"##;
    ...
    std::fs::write("target/vapor_outputs/spec_untyped_arrow.out.js", strip_marker(&out)).ok();
