//! SWC 插件转换行为测试（spec_fn_return_type）
//!
//! 覆盖：此用例的转换快照对比。
...
#[test]
fn transforms_function_decl_with_jsx_return_type() {
    let src = r##"
...
export default PostDetail;"##;
    ...
    std::fs::write("target/vapor_outputs/spec_fn_return_type.out.js", strip_marker(&out)).ok();
    let normalized = normalize(&strip_marker(&out));
    assert!(normalized.contains("function PostDetail(): JSX.Element"), "{normalized}");
