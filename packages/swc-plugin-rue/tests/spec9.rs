...
"##;
    ...
    assert!(normalized.contains("_$compiledComponent(Hello, ()=>({}))"));
    assert!(normalized.contains("_$compiledComponent(World, ()=>({}))"));
    assert_eq!(normalized.matches("_$mountCompiledSlotAt(").count(), 2);
