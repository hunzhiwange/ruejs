...
`
  ...
  it('updates all Rue reactive sources through their precise internal capability entries', () => {
    compiledRuntime.setReactiveScheduling('sync')
    runtimeRoot.setReactiveScheduling('sync')
    ...
    for (const reactiveCase of cases) {
      ...
      expect(esm, reactiveCase.name).toContain('@rue-js/rue/internal/dom')
      expect(esm, reactiveCase.name).toContain('@rue-js/rue/internal/reactive')
      expect(esm, reactiveCase.name).toContain('@rue-js/rue/internal/block')
  ...
  it('updates a props conditional index-key list but currently mounts an empty region', async () => {
    const source = `
    ...
    expect(host.textContent).toBe('0 videosnone')
    props.update({
      ...props.snapshot(),
      videos: [
    ...
    await Promise.resolve()
    expect(host.textContent).toBe('2 videosonefirsttwosecond')
    const initialRows = Array.from(host.querySelectorAll('li'))
    ...
    await Promise.resolve()
    const patchedRows = Array.from(host.querySelectorAll('li'))
    expect(patchedRows[0]).toBe(initialRows[0])
    expect(patchedRows[1]).toBe(initialRows[1])
    expect(host.textContent).toBe('2 videosTWOSECONDONEFIRST')
  ...
  it('refreshes a same-path component branch but currently retains the stale snapshot', () => {
    const source = `
