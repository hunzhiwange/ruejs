...
*/
import { onUnmounted, ref, render, useSetup, watch, type FC } from '@rue-js/rue'
...
const useToastMessage = (options: ToastUseMessageOptions = {}) => {
  ...
  const syncViewport = () => {
    ...
    viewportApp = render(
      <ToastMessageViewport records={readRecords()} onDestroy={destroy} {...readOptions()} />,
      viewportElement,
    )
  }
...
const RenderDefaultItemIcon = ({ arg0: type }: { arg0?: ToastItemType }) => {
...
const ToastContent: FC<ToastPartProps> = ({ as = 'div', className, children, ...rest }) => {
  const Component = as as any
  return Component === 'div' ? (
    <div {...rest} className={mergeClassNames('min-w-0 flex-1', className)}>
      {children}
    </div>
  ) : Component === 'span' ? (
    <span {...rest} className={mergeClassNames('min-w-0 flex-1', className)}>
      {children}
    </span>
  ) : Component === 'button' ? (
    <button {...rest} className={mergeClassNames('min-w-0 flex-1', className)}>
      {children}
    </button>
  ) : (
    <></>
...
const ToastTitle: FC<ToastPartProps> = ({ as = 'div', className, children, ...rest }) => {
  const Component = as as any
  return Component === 'div' ? (
    <div {...rest} className={mergeClassNames('font-semibold leading-5', className)}>
      {children}
    </div>
  ) : Component === 'span' ? (
    <span {...rest} className={mergeClassNames('font-semibold leading-5', className)}>
      {children}
    </span>
  ) : Component === 'button' ? (
    <button {...rest} className={mergeClassNames('font-semibold leading-5', className)}>
      {children}
    </button>
  ) : (
    <></>
...
const ToastDescription: FC<ToastPartProps> = ({ as = 'div', className, children, ...rest }) => {
  const Component = as as any
  return Component === 'div' ? (
    <div {...rest} className={mergeClassNames('mt-1 text-xs leading-5 opacity-80', className)}>
      {children}
    </div>
  ) : Component === 'span' ? (
    <span {...rest} className={mergeClassNames('mt-1 text-xs leading-5 opacity-80', className)}>
      {children}
    </span>
  ) : Component === 'button' ? (
    <button {...rest} className={mergeClassNames('mt-1 text-xs leading-5 opacity-80', className)}>
      {children}
    </button>
  ) : (
    <></>
...
const ToastAction: FC<ToastPartProps> = ({ as = 'div', className, children, ...rest }) => {
  const Component = as as any
  return Component === 'div' ? (
    <div {...rest} className={mergeClassNames('flex shrink-0 items-center gap-2', className)}>
      {children}
    </div>
  ) : Component === 'span' ? (
    <span {...rest} className={mergeClassNames('flex shrink-0 items-center gap-2', className)}>
      {children}
    </span>
  ) : Component === 'button' ? (
    <button {...rest} className={mergeClassNames('flex shrink-0 items-center gap-2', className)}>
      {children}
    </button>
  ) : (
    <></>
...
const ToastItem: FC<ToastItemProps> = (
  ...
  const currentOpenState = ref(isControlled ? !!open : itemCtx.uncontrolledOpen.value)
  ...
  const userOnClick = componentProps.onClick
  const resolvedOpen = isControlled ? !!open : currentOpenState.value
  ...
  const ItemIcon = () =>
    slots.icon ? (
      ...
      <RenderDefaultItemIcon arg0={type} />
    )
  ...
  const hasAction = slots.action != null || hasRenderableContent(action)
  const hasIcon = showIcon !== false && icon !== null
          ...
          >
            <div className="flex items-start gap-3">
              ...
              {hasAction || closable ? (
                ...
                >
                  {slots.action ? <>{slots.action}</> : <>{String(action ?? '')}</>}
                  {closable ? (
                    <button
                      type="button"
                      aria-label="关闭提示"
...
const Toast: FC<ToastProps> = ({
  ...
  return Component === 'section' ? (
    ...
    <div {...rest} className={cls} style={Object.keys(mergedStyle).length ? mergedStyle : style}>
      {children}
    </div>
  ) : Component === 'span' ? (
    <span {...rest} className={cls} style={Object.keys(mergedStyle).length ? mergedStyle : style}>
      {children}
    </span>
  ) : Component === 'button' ? (
    <button {...rest} className={cls} style={Object.keys(mergedStyle).length ? mergedStyle : style}>
      {children}
    </button>
  ) : (
    <></>
...
export const ToastHolder: FC<{ state: { attach: (element: HTMLDivElement | null) => void } }> = ({
  state,
}) => <div style={{ display: 'contents' }} ref={state.attach} />
