...
import { probeAppPage, probeAppPageBeforeRender } from '../src/server/app-page-probe.js'
...
// `await params` (Text.js 15+) and `params.id` (pre-15).
...
describe('app page probe helpers', () => {
  ...
  it('turns special page probe failures into immediate responses', async () => {
    ...
    const renderPageSpecialError = vi.fn(async () => new Response('page-fallback', { status: 307 }))
  ...
  it('turns non-special page probe failures into error boundary responses when provided', async () => {
    const pageError = new Error('page failed')
    const renderPageErrorBoundary = vi.fn(
      async () => new Response('error-boundary', { status: 200 }),
    )
    ...
    expect(renderPageErrorBoundary).toHaveBeenCalledWith(pageError)
    expect(result.response?.status).toBe(200)
    await expect(result.response?.text()).resolves.toBe('error-boundary')
  })

  it('leaves client reference probe failures to the real render path', async () => {
    const renderPageErrorBoundary = vi.fn()
    ...
    expect(result.response).toBeNull()
    expect(renderPageErrorBoundary).not.toHaveBeenCalled()
  })

  it('leaves Rue runtime DOM probe failures to the real render path', async () => {
    const renderPageErrorBoundary = vi.fn()
  ...
  // (Text.js 15+ pattern) threw TypeError during probe, causing the probe to
  ...
  it('detects notFound() from an async-params page when params are thenable', async () => {
    const NOT_FOUND_ERROR = new Error('TEXT_NOT_FOUND')
  ...
  it('detects redirect() from an async-searchParams page when searchParams are thenable', async () => {
    const REDIRECT_ERROR = new Error('TEXT_REDIRECT')
  ...
  it('skips the page probe when a loading boundary is present (special errors handled post-shell)', async () => {
    ...
    // rscErrorTracker captures the digest from Rue's onError, and a short
...
describe('probeAppPage', () => {
  ...
  it('lets redirect()/notFound() throws propagate so the probe lifecycle can catch them', async () => {
    const REDIRECT = new Error('TEXT_REDIRECT')
