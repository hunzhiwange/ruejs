...
} from '../src/server/app-rsc-cache-busting.js'
import { devOnCaughtError, devOnUncaughtError } from '../src/server/dev-error-overlay.js'
import {
  AppElementsWire,
...
import { createClientNavigationRenderSnapshot } from '../src/shims/navigation.js'
import * as navigationShim from '../src/shims/navigation.js'
import {
  ...
  type AppRouterState,
  type OperationLane,
} from '../src/server/app-browser-state.js'
import { resolveRscRedirectLifecycleHop } from '../src/server/app-browser-rsc-redirect.js'
import {
  applyApprovedVisibleCommit,
  approveHmrVisibleCommit,
  approvePendingNavigationCommit,
  resolveAndClassifyNavigationCommit,
} from '../src/server/app-browser-visible-commit.js'
import {
  NAVIGATION_TRACE_SCHEMA_VERSION,
  NavigationTraceReasonCodes,
  NavigationTraceTransactionCodes,
  createNavigationTrace,
} from '../src/server/navigation-trace.js'
import {
  ACTION_REVALIDATED_HEADER,
  TEXT_INTERCEPTION_CONTEXT_HEADER,
...
import type {
  ...
  StaticSegmentGraph,
} from '../src/routing/app-route-graph.js'
...
function createRouteManifestForPendingCommit(
  ...
  if (pendingInterception !== null) {
    ...
    const sourceLayoutIds = currentMatchesSource ? currentState.layoutIds : pending.action.layoutIds
...
type TestPendingDispositionOptions = {
  ...
  textRootLayoutTreePath: string | null
  renderId?: number
...
async function resolveTestPendingNavigationCommitDispositionDecision(
  ...
  const pending = await createPendingNavigationCommit({
    ...
    textElements: Promise.resolve(
      createResolvedElements('route:/dashboard', options.textRootLayoutTreePath),
    ),
...
async function applyApprovedTestCommit(
  ...
  const pending = await createPendingNavigationCommit({
    ...
    previousTextUrl: options.previousTextUrl,
...
describe('app browser entry navigation scheduling', () => {
  ...
  it('creates replayable cached RSC snapshots with compatibility IDs', async () => {
    ...
    const snapshot = navigationShim.createCachedRscResponseSnapshot(
      new Response('unused', {
        headers: {
          ...
          [TEXT_MOUNTED_SLOTS_HEADER]: 'children',
          [TEXT_PARAMS_HEADER]: '%7B%22slug%22%3A%22target%22%7D',
          [TEXT_RSC_COMPATIBILITY_ID_HEADER]: 'compat-a',
        },
      }),
    ...
    expect(replayed.headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-a')
    expect(replayed.headers.get(TEXT_MOUNTED_SLOTS_HEADER)).toBe('children')
    expect(replayed.headers.get(TEXT_PARAMS_HEADER)).toBe('%7B%22slug%22%3A%22target%22%7D')
  ...
  it('does not expose a per-navigation transition override at the controller boundary', () => {
    ...
    function assertNoTransitionOverride(controller: Controller) {
      void controller.renderNavigationPayload({
        ...
        navigationSnapshot: createClientNavigationRenderSnapshot('https://example.com/initial', {}),
        textElements: Promise.resolve(createResolvedElements('route:/dashboard', '/')),
        operationLane: 'navigation',
        payloadOrigin: FRESH_APP_NAVIGATION_PAYLOAD_ORIGIN,
        params: {},
...
describe('app browser entry state helpers', () => {
  ...
  it('merges elements on approved navigate commits', async () => {
    ...
    const textState = await applyApprovedTestCommit(state, {
      extraEntries: {
        'page:/text': createElement('main', null, 'text'),
      },
      rootLayoutTreePath: '/',
      routeId: 'route:/text',
    })

    expect(textState.routeId).toBe('route:/text')
    expect(textState.interceptionContext).toBeNull()
    expect(textState.previousTextUrl).toBeNull()
    expect(textState.rootLayoutTreePath).toBe('/')
    expect(textState.visibleCommitVersion).toBe(1)
    expect(textState.activeOperation).toMatchObject({
      id: 1,
      lane: 'navigation',
    ...
    expect(textState.elements).toMatchObject({
      'layout:/': expect.anything(),
      'page:/text': expect.anything(),
    })
  })

  it('replaces elements on approved replace commits', async () => {
    const textElements = createResolvedElements('route:/text', '/', null, {
      'page:/text': createElement('main', null, 'text'),
    })
    ...
    const pending = await createPendingNavigationCommit({
      payloadOrigin: FRESH_APP_NAVIGATION_PAYLOAD_ORIGIN,
      currentState: state,
      textElements: Promise.resolve(textElements),
    ...
    const approval = approvePendingNavigationCommit({
      ...
      targetHref: 'https://example.com/text',
    })
    if (approval.approvedCommit === null) {
      throw new Error('Expected approved visible commit')
    }

    const textState = applyApprovedVisibleCommit(state, approval.approvedCommit)

    expect(textState.elements).toBe(textElements)
    expect(textState.interceptionContext).toBeNull()
    expect(textState.previousTextUrl).toBeNull()
    expect(textState.elements).toMatchObject({
      'page:/text': expect.anything(),
  ...
  it('does not export a raw visible state reducer outside the approved commit boundary', async () => {
    const stateModule = await import('../src/server/app-browser-state.js')
  ...
  it('carries interception context through pending navigation commits', async () => {
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          '/feed',
          {
            'page:/photos/42': createElement('main', null, 'photo'),
          },
    ...
    expect(pending.previousTextUrl).toBe('/feed')
    ...
    expect(pending.action.previousTextUrl).toBe('/feed')
  })

  it('clears previousTextUrl when traversing to a non-intercepted entry', async () => {
    ...
    // and passes previousTextUrl: null explicitly — meaning "not intercepted".
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(createResolvedElements('route:/feed', '/')),
      navigationSnapshot: createState().navigationSnapshot,
    ...
    expect(pending.previousTextUrl).toBeNull()
    expect(pending.action.previousTextUrl).toBeNull()
  })
  ...
  it('hard navigates instead of merging when the root layout changes', async () => {
    ...
    const pending = await createPendingNavigationCommit({
      payloadOrigin: FRESH_APP_NAVIGATION_PAYLOAD_ORIGIN,
      currentState,
      textElements: Promise.resolve(createResolvedElements('route:/dashboard', '/(dashboard)')),
  ...
  it('defers commit classification until the payload has resolved', async () => {
    ...
    const textElements = new Promise<AppElements>(resolve => {
    ...
    resolveElements(
      normalizeAppElements({
        ...
        [APP_ROOT_LAYOUT_KEY]: '/',
        'page:/dashboard': createElement('main', null, 'dashboard'),
      }),
    )
  ...
  it('skips a traverse commit when visible state changed after traversal started', async () => {
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(createResolvedElements('route:/previous', '/')),
  ...
  it('traces stale pending commits with compact reason codes and structured fields', async () => {
    const decision = await resolveTestPendingNavigationCommitDispositionDecision({
      ...
      currentRootLayoutTreePath: '/',
      textRootLayoutTreePath: '/(dashboard)',
      startedNavigationId: 4,
      startedVisibleCommitVersion: 0,
    })
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          null,
          {
            'page:/dashboard/settings': createElement('main', null, 'settings'),
          },
          ['layout:/'],
        ),
      ),
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          '/caf%C3%A9',
          {
            'page:/photos/café': createElement('main', null, 'photo'),
          },
  ...
  it('builds a merge commit for refresh and server-action payloads', async () => {
    ...
    expect(refreshCommit.previousTextUrl).toBeNull()
  })
  ...
  it('commits non-intercepted context-only payloads without preserving stale interception state', async () => {
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          '/feed',
          {
            [AppElementsWire.encodePageId('/feed', '/feed')]: createElement('main', null, 'feed'),
          },
    ...
    expect(contextOnlyState.previousTextUrl).toBeNull()
    ...
    const interceptedPending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          '/feed',
          {
            [AppElementsWire.encodeRouteId('/photos/42', '/feed')]: createElement(
              'main',
              null,
              'photo',
            ),
            [AppElementsWire.encodeSlotId('modal', '/feed')]: createElement('div', null, 'modal'),
          },
          [AppElementsWire.encodeLayoutId('/'), AppElementsWire.encodeLayoutId('/feed')],
          [
            {
  ...
  it('creates an approved visible commit only after the current operation decision allows mutation', async () => {
    ...
    const textState = applyApprovedVisibleCommit(currentState, approval.approvedCommit)
    expect(textState.routeId).toBe('route:/dashboard')
    expect(textState.visibleCommitVersion).toBe(1)
    expect(textState.activeOperation).toMatchObject({
      id: 11,
      lane: 'navigation',
  ...
  it('traces unknown root-layout approval as an unproven payload commit', async () => {
    ...
    const pending = await createPendingNavigationCommit({
      payloadOrigin: FRESH_APP_NAVIGATION_PAYLOAD_ORIGIN,
      currentState,
      textElements: Promise.resolve(createResolvedElements('route:/legacy-payload', null)),
      navigationSnapshot: currentState.navigationSnapshot,
      operationLane: 'navigation',
      renderId: 16,
      type: 'navigate',
  ...
  it('approves HMR visible commits through a named trusted recovery path', async () => {
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements('route:/hmr', '/', null, {
          'page:/hmr': createElement('main', null, 'hmr'),
        }),
      ),
      navigationSnapshot: currentState.navigationSnapshot,
      operationLane: 'hmr',
    ...
    const textState = applyApprovedVisibleCommit(currentState, approvedCommit)

    expect(textState.routeId).toBe('route:/hmr')
    expect(textState.activeOperation).toMatchObject({
      id: 14,
      lane: 'hmr',
      state: 'committed',
  ...
  it('rejects non-HMR commits on the HMR approval path', async () => {
    ...
    expect(() => approveHmrVisibleCommit(pending)).toThrow(
      '[text] HMR visible commit approval requires an HMR pending operation',
    )
  })
  ...
  it('applies approved replace commits without preserving old elements', async () => {
    const currentState = createState({
      elements: createResolvedElements('route:/initial', '/', null, {
        'layout:/old': createElement('div', null, 'old'),
      }),
    })
    const textElements = createResolvedElements('route:/text', '/', null, {
      'page:/text': createElement('main', null, 'text'),
    ...
    const textState = applyApprovedVisibleCommit(currentState, approval.approvedCommit)
    expect(textState.elements).toBe(textElements)
    expect(Object.hasOwn(textState.elements, 'layout:/old')).toBe(false)
    expect(textState.activeOperation).toMatchObject({
      id: 12,
      lane: 'navigation',
      state: 'committed',
  ...
  it('applies approved traverse commits with stale slot cleanup', async () => {
    const currentState = createState({
      elements: createResolvedElements('route:/feed/comments', '/', null, {
        'slot:modal:/feed': createElement('div', null, 'modal'),
      }),
    })
    const pending = await createPendingNavigationCommit({
      payloadOrigin: FRESH_APP_NAVIGATION_PAYLOAD_ORIGIN,
    ...
    const textState = applyApprovedVisibleCommit(currentState, approval.approvedCommit)
    expect(textState.routeId).toBe('route:/feed')
    expect(Object.hasOwn(textState.elements, 'slot:modal:/feed')).toBe(false)
    expect(textState.activeOperation).toMatchObject({
      id: 13,
      lane: 'traverse',
      state: 'committed',
  ...
  it('preserves layoutFlags only for approved same-layout ancestors', async () => {
    ...
    expect(textState.layoutFlags).toEqual({
      'layout:/': 's',
      'layout:/blog': 'd',
    })
  })
  ...
  it('replaces layoutFlags on approved replace commits', async () => {
    ...
    expect(textState.layoutFlags).toEqual({ 'layout:/': 'd' })
  })

  it('stores previousTextUrl on approved navigate commits', async () => {
    const state = createState({
      layoutIds: [AppElementsWire.encodeLayoutId('/'), AppElementsWire.encodeLayoutId('/feed')],
      navigationSnapshot: createClientNavigationRenderSnapshot('https://example.com/feed', {}),
      routeId: 'route:/feed',
    ...
    expect(textState.interception).toEqual(interception)
    expect(textState.interceptionContext).toBe('/feed')
    expect(textState.previousTextUrl).toBe('/feed')
  })
})
...
describe('app browser navigation controller', () => {
  ...
  it('skips stale browser navigations before committing their payload', async () => {
    ...
    const createNavigationCommitEffect = vi.fn(() => vi.fn())
    let resolveTextElements: ((value: AppElements) => void) | undefined
    const textElements = new Promise<AppElements>(resolve => {
      resolveTextElements = resolve
    })
  ...
  it('dispatches same-URL server action payloads into the browser router state', async () => {
    ...
    const textElements = Promise.resolve(
      createResolvedElements('route:/settings/account', '/', null, {
        'page:/settings/account': createElement('main', null, 'account'),
      }),
    )
    ...
    try {
      ...
      expect(stateRef.current.previousTextUrl).toBeNull()
  ...
  it('syncs cleared previousTextUrl after same-URL server action commits', async () => {
    const interception = createInterceptionProof('/feed', '/photos/42')
    const initialState = createState({
      interception,
      interceptionContext: '/feed',
    ...
    const syncHistoryStatePreviousTextUrl = vi.fn()
    const { controller, detach, stateRef } = createControllerHarness(initialState, {
      syncHistoryStatePreviousTextUrl,
    })
    const { assign } = stubWindow('https://example.com/photos/42')
    const textElements = Promise.resolve(
      createResolvedElements('route:/photos/42', '/', null, {
        'page:/photos/42': createElement('main', null, 'photo page'),
      }),
    )
    ...
    try {
      ...
      expect(stateRef.current.previousTextUrl).toBeNull()
      expect(syncHistoryStatePreviousTextUrl).toHaveBeenCalledTimes(1)
      expect(syncHistoryStatePreviousTextUrl).toHaveBeenCalledWith(null)
    } finally {
      detach()
    }
  })
    ...
    try {
      ...
      const newerResult = await controller.commitSameUrlNavigatePayload(
        Promise.resolve(
          createResolvedElements('route:/settings/newer', '/', null, {
            'page:/settings/newer': createElement('main', null, 'newer'),
          }),
        ),
        stateRef.current.navigationSnapshot,
        {
      ...
      resolveOlderPayload(
        createResolvedElements('route:/settings/older', '/', null, {
          'page:/settings/older': createElement('main', null, 'older'),
        }),
      )
    ...
    try {
      ...
      resolvePayload(
        createResolvedElements('route:/settings/action', '/', null, {
          'page:/settings/action': createElement('main', null, 'action'),
        }),
      )
  ...
  it('most recent navigation commits when three are started and payloads resolve in reverse order', async () => {
    ...
    const payloadC = Promise.resolve(
      createResolvedElements('route:/c', '/', null, {
        'page:/c': createElement('main', null, 'C'),
      }),
    )
    ...
    try {
      ...
      resolveB(
        createResolvedElements('route:/b', '/', null, {
          'page:/b': createElement('main', null, 'B'),
        }),
      )
      await Promise.resolve()
      await Promise.resolve()
      ...
      resolveA(
        createResolvedElements('route:/a', '/', null, {
          'page:/a': createElement('main', null, 'A'),
        }),
      )
      await Promise.resolve()
      await Promise.resolve()
      ...
      void controller.renderNavigationPayload({
        ...
        textElements: Promise.resolve(
          createResolvedElements('route:/marketing/settings', '/(marketing)', null, {
            'page:/marketing/settings': createElement('main', null, 'settings'),
          }),
        ),
        operationLane: 'navigation',
        params: {},
    ...
    try {
      ...
      resolveRefresh(
        createResolvedElements('route:/initial-refreshed', '/', null, {
          'page:/initial': createElement('main', null, 'refreshed'),
        }),
      )
      await Promise.resolve()
      await Promise.resolve()
    ...
    try {
      ...
      resolveTraverse(
        createResolvedElements('route:/previous', '/', null, {
          'page:/previous': createElement('main', null, 'previous'),
        }),
      )
      await Promise.resolve()
      await Promise.resolve()
  ...
  it('failed payload cleanly settles the pending router state without leaving it hanging', async () => {
    ...
    try {
      ...
      const renderPromise = controller.renderNavigationPayload({
        ...
        historyUpdateMode: 'push',
        navigationSnapshot: createClientNavigationRenderSnapshot('https://example.com/initial', {}),
        textElements: Promise.reject(new Error('RSC fetch failed')),
        operationLane: 'navigation',
        params: {},
        pendingRouterState,
        previousTextUrl: null,
...
describe('app browser entry previousTextUrl helpers', () => {
  it('stores previousTextUrl alongside existing history state', () => {
    expect(
      createHistoryStateWithPreviousTextUrl(
        {
          __text_scrollY: 120,
      ...
      __text_previousTextUrl: '/feed?tab=latest',
      __text_scrollY: 120,
    })
  })

  it('drops previousTextUrl when cleared', () => {
    expect(
      createHistoryStateWithPreviousTextUrl(
        {
          __text_previousTextUrl: '/feed',
          __text_scrollY: 120,
        },
        null,
      ),
  ...
  it('reads previousTextUrl from history state', () => {
    expect(
      readHistoryStatePreviousTextUrl({
        __text_previousTextUrl: '/feed?tab=latest',
      }),
  ...
  it('derives interception context from previousTextUrl pathname', () => {
    expect(resolveInterceptionContextFromPreviousTextUrl('/feed?tab=latest')).toBe('/feed')
  })

  it('returns null when previousTextUrl is missing', () => {
    expect(readHistoryStatePreviousTextUrl({})).toBeNull()
    expect(resolveInterceptionContextFromPreviousTextUrl(null)).toBeNull()
  })
  ...
  it('stores traversal index alongside existing history state', () => {
    const state = createHistoryStateWithNavigationMetadata(
      ...
      {
        previousTextUrl: '/feed?tab=latest',
        traversalIndex: 4,
      },
    )

    expect(state).toEqual({
      __text_historyIndex: 4,
      __text_previousTextUrl: '/feed?tab=latest',
      __text_scrollY: 120,
    })
  ...
  it('resolves back, forward, and unknown traversal intent from history state', () => {
    expect(
      resolveHistoryTraversalIntent({
        ...
        historyState: { __text_historyIndex: 3 },
      }).direction,
    ).toBe('back')
    expect(
      resolveHistoryTraversalIntent({
        currentHistoryIndex: 5,
        historyState: { __text_historyIndex: 7 },
      }).direction,
    ).toBe('forward')
    expect(
      resolveHistoryTraversalIntent({
  ...
  it('preserves only planner-approved same-layout ancestors on navigate commits', async () => {
    ...
    const staleLayout = createElement('div', null, 'stale layout')
    const stalePage = createElement('main', null, 'stale page')
    const state = createState({
    ...
    expect(textState.elements['layout:/']).toBe(rootLayout)
    expect(textState.elements['layout:/dashboard']).toBe(dashboardLayout)
    expect(Object.hasOwn(textState.elements, 'layout:/stale')).toBe(false)
    expect(Object.hasOwn(textState.elements, 'page:/stale')).toBe(false)
    expect(textState.layoutFlags).toEqual({
      'layout:/': 's',
      'layout:/dashboard': 's',
      'layout:/dashboard/settings': 'd',
    })
    expect(textState.layoutIds).toEqual([
      'layout:/',
      'layout:/dashboard',
  ...
  it('does not preserve same-layout ancestors when root identity is unknown', async () => {
    ...
    const textState = await applyApprovedTestCommit(state, {
      extraEntries: {
        'page:/legacy': createElement('main', null, 'legacy'),
      },
      layoutIds: [],
    ...
    expect(Object.hasOwn(textState.elements, 'layout:/')).toBe(false)
    expect(textState.layoutIds).toEqual([])
  })
  ...
  it('does not approve mounted parallel slots on approved traverse commits', async () => {
    const feedLayout = createElement('div', null, 'feed layout')
    const mountedSlot = createElement('div', null, 'modal')
    ...
    const state = createState({
      elements: createResolvedElements(
        ...
        null,
        {
          'layout:/': createElement('div', null, 'root layout'),
    ...
    const pending = await createPendingNavigationCommit({
      ...
      textElements: Promise.resolve(
        createResolvedElements(
          ...
          null,
          {
            'page:/feed/comments': createElement('main', null, 'comments'),
          },
    ...
    const textState = applyApprovedVisibleCommit(state, approval.approvedCommit)
    expect(textState.elements['layout:/feed']).toBe(feedLayout)
    expect(Object.hasOwn(textState.elements, 'slot:modal:/feed')).toBe(false)
  })
  ...
  it('preserves planner-approved default parallel slots on approved navigate commits', async () => {
    ...
    const state = createState({
      elements: createResolvedElements(
        ...
        {
          'layout:/': createElement('div', null, 'root layout'),
          'layout:/feed': createElement('div', null, 'feed layout'),
          'slot:modal:/feed': mountedSlot,
        },
        ['layout:/', 'layout:/feed'],
        [modalSlotBinding],
    ...
    expect(Object.hasOwn(textState.elements, 'slot:modal:/feed')).toBe(true)
    expect(textState.elements['slot:modal:/feed']).toBe(mountedSlot)
    expect(textState.slotBindings).toEqual([modalSlotBinding])
  })
  ...
  it('does not preserve absent parallel slots when their owner layout is not approved', async () => {
    ...
    const textState = await applyApprovedTestCommit(state, {
      extraEntries: {
        'page:/settings': createElement('main', null, 'settings'),
      },
...
describe('createPopstateRestoreHandler', () => {
  it('restores scroll only after the latest popstate navigation commits', async () => {
    ...
    window.__TEXT_RSC_PENDING__ = null
    ...
    handler({ state: { __text_scrollY: 10 } } as PopStateEvent)
    handler({ state: { __text_scrollY: 20 } } as PopStateEvent)

    expect(window.__TEXT_RSC_PENDING__).toBe(secondNavigation.promise)

    secondNavigation.resolve()
    await secondNavigation.promise
    await Promise.resolve()

    expect(restoreCalls).toEqual([{ __text_scrollY: 20 }])
    expect(window.__TEXT_RSC_PENDING__).toBeNull()
  ...
  it('clears __TEXT_RSC_PENDING__ when a stale popstate navigation settles', async () => {
    const restoreCalls: unknown[] = []
    ...
    handler({ state: { __text_scrollY: 10 } } as PopStateEvent)
    expect(window.__TEXT_RSC_PENDING__).toBe(navigation.promise)
...
describe('devOnCaughtError (Rue root dev handler)', () => {
  it('ignores redirect sentinels handled by RedirectBoundary', () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    try {
      devOnCaughtError(
        Object.assign(new Error('TEXT_REDIRECT:/?auth=required'), {
  ...
  it('includes the component stack in the log when provided', () => {
    const consoleSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
    try {
...
describe('devOnUncaughtError (Rue root dev handler)', () => {
...
describe('createOnUncaughtError (Rue root uncaught handler)', () => {
...
describe('app browser form-state hydration', () => {
  ...
  it('passes the one-shot form-state bootstrap payload to Rue root options', () => {
    ...
    const rootOptions = createTextRueRootOptions({
      formState: consumedFormState,
      onCaughtError,
      onUncaughtError,
    })
    mountRoot('document', 'root', rootOptions)
  ...
  it('preserves null form state as an explicit Rue root option', () => {
    const onUncaughtError = vi.fn()
...
describe('mounted slot helpers', () => {
  it('collects only mounted slot ids', () => {
    const elements: AppElements = createResolvedElements('route:/dashboard', '/', null, {
      'layout:/': createElement('div', null, 'layout'),
      'slot:modal:/': createElement('div', null, 'modal'),
      'slot:sidebar:/': createElement('div', null, 'sidebar'),
      'slot:ghost:/': null,
  ...
  it('serializes mounted slot ids into a stable header value', () => {
    const elements: AppElements = createResolvedElements('route:/dashboard', '/', null, {
      'slot:z:/': createElement('div', null, 'z'),
      'slot:a:/': createElement('div', null, 'a'),
    })
...
describe('resolveServerActionRequestState', () => {
  it('includes only the RSC markers and x-rsc-action when previousTextUrl is null and no slots are mounted', () => {
  ...
  it('derives the interception context header from previousTextUrl', () => {
    const elements = createResolvedElements('route:/photos/42', '/')
    const previousTextUrl = '/feed?tab=latest'
    ...
    expect(headers.get(TEXT_INTERCEPTION_CONTEXT_HEADER)).toBe(
      resolveInterceptionContextFromPreviousTextUrl(previousTextUrl, ''),
    )
  })

  it('strips the base path when deriving the interception context', () => {
    const elements = createResolvedElements('route:/photos/42', '/')
    const previousTextUrl = '/app/feed'
    ...
    expect(headers.get(TEXT_INTERCEPTION_CONTEXT_HEADER)).toBe(
      resolveInterceptionContextFromPreviousTextUrl(previousTextUrl, '/app'),
    )
  })

  it('derives the mounted slots header from mounted slot keys', () => {
    const elements: AppElements = createResolvedElements('route:/feed', '/', null, {
      'slot:@modal:/feed': createElement('div', null, 'modal'),
      'slot:@sidebar:/feed': createElement('div', null, 'sidebar'),
    })
    ...
    expect(headers.get(TEXT_MOUNTED_SLOTS_HEADER)).toBe(getMountedSlotIdsHeader(elements))
  })
  ...
  it('omits headers whose derived values are null', () => {
    ...
    expect(headers.has(TEXT_INTERCEPTION_CONTEXT_HEADER)).toBe(false)
    expect(headers.has(TEXT_MOUNTED_SLOTS_HEADER)).toBe(false)
  })
})
