...
} from '../src/server/app-page-boundary-render.js'
import type { AppElements } from '../src/server/app-elements.js'
import type { MetadataFileRoute } from '../src/server/metadata-routes.js'
import { TEXT_RSC_VARY_HEADER } from '../src/server/app-rsc-cache-busting.js'
import {
  createElement,
  createStreamFromMarkup,
...
function RootLayout({ children, params }: { children: TestServerNode; params: { slug?: string } }) {
  return createElement('div', { 'data-layout': 'root', 'data-slug': params.slug }, children)
}

function LeafLayout({ children, params }: { children: TestServerNode; params: { slug?: string } }) {
  return createElement('section', { 'data-layout': 'leaf', 'data-slug': params.slug }, children)
}
...
function GlobalErrorBoundary({ error }: { error: Error }) {
  return createElement('p', { 'data-boundary': 'global-error' }, `global:${error.message}`)
}
...
describe('app page boundary render helpers', () => {
  ...
  it('renders HTTP access fallbacks with layout metadata and wrapped HTML', async () => {
    ...
    expect(html).toMatch(/<meta\b(?=[^>]*name="theme-color")(?=[^>]*content="#111111")/)
  ...
  it('preserves middleware headers on HTTP access fallback HTML responses', async () => {
    ...
    expect(response?.headers.get('vary')).toBe(`${TEXT_RSC_VARY_HEADER}, x-auth-state`)
  ...
  it('renders error boundaries when dynamic file metadata resolution fails', async () => {
    ...
    try {
      ...
      expect(consoleError).toHaveBeenCalledWith(
        '[text] File-based metadata resolution failed while rendering error boundary for /posts/[slug]:',
        error,
      )
