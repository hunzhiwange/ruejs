...
} from '../src/server/app-server-action-execution.js'
import {
  createServerActionNotFoundResponse,
  throwOnServerActionNotFound,
} from '../src/server/server-action-not-found.js'
import { unstable_isUnrecognizedActionError } from '../src/shims/navigation.js'
import {
  TEXT_RSC_COMPATIBILITY_ID_HEADER,
  TEXT_RSC_VARY_HEADER,
} from '../src/server/app-rsc-cache-busting.js'
import { refresh, revalidatePath, revalidateTag } from '../src/shims/cache.js'
import { setHeadersAccessPhase } from '../src/shims/headers.js'
...
function createMultipartRequest(headers?: HeadersInit): Request {
  const requestHeaders = new Headers({
    'content-type': 'multipart/form-data; boundary=text',
    host: 'example.com',
...
describe('app server action execution helpers', () => {
  ...
  it('identifies progressive multipart server action submissions', () => {
    expect(
      isProgressiveServerActionRequest(
        { method: 'post' },
        'multipart/form-data; boundary=text',
        null,
      ),
    ).toBe(true)
    expect(
  ...
  it('rejects malformed action payloads before decoding the action', async () => {
    const formData = new FormData()
    formData.set('0', '"$Q1"')
    const decodeProgressiveAction = vi.fn()
    ...
    expect(decodeProgressiveAction).not.toHaveBeenCalled()
  })
  ...
  it('executes decoded form actions and converts redirects into 303 responses', async () => {
    ...
    const clearContext = vi.fn()
    const formData = new FormData()
    formData.set('$RUE_ACTION_ID_test', '')

    const response = requireProgressiveActionResponse(
      await handleProgressiveServerActionRequest(
        createOptions({
          clearRequestContext: clearContext,
          async decodeProgressiveAction(body) {
            expect(body).toBe(formData)
            return () => {
              throw { digest: 'TEXT_REDIRECT;replace;%2Fresult%3Fok%3D1;307' }
            }
          },
          getAndClearPendingCookies() {
    ...
    const result = await handleProgressiveServerActionRequest(
      createOptions({
        ...
        async decodeFormState(actionResult, body) {
          ...
          expect(body.get('$RUE_ACTION_ID_test')).toBe('')
  ...
  it('passes HTTP fallback errors as actionError to be rendered by error boundaries', async () => {
    for (const digest of ['TEXT_NOT_FOUND', 'TEXT_HTTP_ERROR_FALLBACK;403']) {
      const clearContext = vi.fn()
  ...
  it('returns the action-not-found response for progressive action decode misses', async () => {
    ...
    const response = requireProgressiveActionResponse(
      await handleProgressiveServerActionRequest(
        createOptions({
          ...
          async decodeProgressiveAction() {
            throw new Error(
              'Failed to find Server Action. This request might be from an older or newer deployment.\nRead more: https://textjs.org/docs/messages/failed-to-find-server-action',
            )
          },
          reportRequestError(error) {
            reportedErrors.push(error)
    ...
    expect(response.headers.get('x-textjs-action-not-found')).toBe('1')
  ...
  it('executes fetch actions and returns a rerendered RSC payload', async () => {
    ...
    const actionReferences = { marker: 'test-refs' }
    ...
    const response = await handleServerActionRscRequest(
      createRscOptions({
        ...
        parseActionArgs(body, options) {
          expect(body).toBe('encoded-flight-body')
          expect(options.references).toBe(actionReferences)
    ...
    expect(response?.headers.get('vary')).toBe(TEXT_RSC_VARY_HEADER)
    expect(response?.headers.get('x-middleware')).toBe('present')
  ...
  it('skips page rerendering for fetch actions that do not revalidate', async () => {
    ...
    await withEnvVar('__TEXT_RSC_COMPATIBILITY_ID', 'compat-action', async () => {
      const response = await handleServerActionRscRequest(
        createRscOptions({
          buildPageElement,
          middlewareHeaders: new Headers([[TEXT_RSC_COMPATIBILITY_ID_HEADER, 'spoofed-compat']]),
      ...
      expect(response?.status).toBe(200)
      expect(response?.headers.get('content-type')).toBe('text/x-component')
      expect(response?.headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-action')
  ...
  // Mirrors Text.js' action revalidation header contract:
  // packages/text/src/server/app-render/action-handler.ts addRevalidationHeader()
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/action-handler.ts
  ...
  // Ported from Text.js: test/e2e/app-dir/actions/app-action.test.ts
  ...
  it('rejects malformed fetch-action payloads before parseActionArgs', async () => {
    const parseActionArgs = vi.fn()
    ...
    expect(response?.status).toBe(400)
    expect(await response?.text()).toBe('Invalid server action payload')
    expect(parseActionArgs).not.toHaveBeenCalled()
  })

  // Ported from Text.js: test/e2e/app-dir/no-server-actions/no-server-actions.test.ts
  // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/no-server-actions/no-server-actions.test.ts
  it('returns the Text.js action-not-found response for stale fetch action ids', async () => {
    const parseActionArgs = vi.fn()
    const renderToReadableStream = vi.fn()
  ...
  it('returns action-not-found for stale Rue server action ids', async () => {
    const parseActionArgs = vi.fn()

    const response = await handleServerActionRscRequest(
      createRscOptions({
        actionId: 'stale-rue-action',
        parseActionArgs,
        loadServerAction() {
          return Promise.reject(new Error("[rue-rsc] invalid server reference 'stale-rue-action'"))
        },
      }),
    )
  ...
  // `x-textjs-action-not-found` response that the client router recognises.
  //
  // Ported from Text.js: test/e2e/app-dir/no-server-actions/no-server-actions.test.ts
  // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/no-server-actions/no-server-actions.test.ts
  ...
  it('encodes fetch-action redirects as RSC control headers', async () => {
    ...
    const response = await handleServerActionRscRequest(
      createRscOptions({
        ...
        loadServerAction() {
          return Promise.resolve(() => {
            throw { digest: 'TEXT_REDIRECT;;%2Ftarget%3Fok%3D1;308' }
          })
        },
        middlewareHeaders: new Headers([['x-middleware', 'present']]),
        renderToReadableStream,
      ...
      createRscOptions({
        loadServerAction() {
          return Promise.resolve(async () => {
            ...
            throw { digest: 'TEXT_REDIRECT;;%2Ftarget;307' }
          })
        },
      }),
    )
  ...
  // Ported from Text.js: vercel/text.js@20892dd
  ...
  // Ported from Text.js: packages/text/src/server/app-render/action-handler.ts
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/action-handler.ts
  it('sets the HTTP fallback status while packaging fallback digests into fetch-action payload', async () => {
    for (const [digest, statusCode] of [
      ['TEXT_NOT_FOUND', 404],
      ['TEXT_HTTP_ERROR_FALLBACK;404', 404],
      ['TEXT_HTTP_ERROR_FALLBACK;403', 403],
    ]) {
      let renderedModel: TestActionModel | null = null
  ...
  it('emits the `x-edge-runtime: 1` marker on action redirect responses for edge-runtime routes', async () => {
    const response = await handleServerActionRscRequest(
      createRscOptions({
        ...
        loadServerAction() {
          return Promise.resolve(() => {
            throw { digest: 'TEXT_REDIRECT;;%2Fdashboard;307' }
          })
        },
      }),
    )
...
// on the `x-textjs-action-not-found` response header:
