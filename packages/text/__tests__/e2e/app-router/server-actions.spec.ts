...
test.describe('Server Actions', () => {
  ...
  test('message form calls server action with FormData', async ({ page }) => {
    ...
    await page.fill('[data-testid="message-input"]', 'Hello text!')
  ...
  test('server action formData redirect works without JavaScript', async ({ browser }) => {
    ...
    page.on('response', response => {
      ...
      if (url.pathname === '/textjs-compat/action-progressive') {
        actionResponseStatus = response.status()
      }
    })

    try {
      await page.goto(`${BASE}/textjs-compat/action-progressive`)
      await page.fill('#name', 'test')
      await page.click('#submit')

      await expect(page).toHaveURL(
        `${BASE}/textjs-compat/action-progressive/result?name=test&hidden-info=hi`,
      )
  ...
  test('server action cookie writes do not make the rerender path mutable', async ({ page }) => {
    test.slow()
    await page.goto(`${BASE}/textjs-compat/action-cookie-phase`)
...
test.describe('useActionState', () => {
  ...
  // Ported from Text.js' progressive action form-state path:
  // packages/text/src/server/app-render/action-handler.ts decodes form state
  // and packages/text/src/server/app-render/use-flight-response.tsx serializes
  // it for hydrateRoot().
  ...
  test('useActionState counter increments via server action', async ({ page }) => {
    ...
    // Wait for hydration
    await expect(async () => {
      const ready = await page.evaluate(() => !!(window as any).__TEXT_RSC_ROOT__)
...
test.describe('Server action forwarding loop guard', () => {
  test('middleware rewrite of action POST does not hang (no forwarding loop)', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/action-forward-loop`)
    ...
    // but text's single-worker bundle still finds the action locally.
  ...
  // x-action-forwarded guard. In text's single-worker model the action is
  // found locally, so the guard cannot be triggered organically via E2E.
  test('stale action ID returns 404 with x-textjs-action-not-found header', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/action-forward-loop`)
    await waitForAppRouterHydration(page)

    const response = await page.evaluate(async base => {
      const res = await fetch(`${base}/textjs-compat/action-forward-loop`, {
        method: 'POST',
        headers: {
      ...
      return {
        status: res.status,
        hasNotFoundHeader: res.headers.get('x-textjs-action-not-found') === '1',
      }
    }, BASE)
