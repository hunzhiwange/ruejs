...
import type { AppPageModule } from '../src/server/app-page-route-wiring.js'
import type { AppPageParams } from '../src/server/app-page-boundary.js'
...
// eslint-disable-text-line import/first
import { buildPageElements } from '../src/server/app-page-element-builder.js'
import type { AppPageBuildRoute } from '../src/server/app-page-element-builder.js'
...
describe('buildPageElements', () => {
  ...
  it('keeps client page references as element types instead of invoking them on the server', async () => {
    const clientPageReference = Object.assign(
      () => {
        throw new Error('client page reference should not execute on the server')
      },
      {
        $$typeof: AppRscServerClientReferenceSymbol,
        $$id: '/app/client-page.tsx#default',
      },
    )
    const route = createSyntheticRoute({
      page: createSyntheticPageModule(clientPageReference),
      pattern: '/client-page',
    })
  ...
  it('constructs a full element tree for a page with a default export', async () => {
    function TestPage(): RenderableOutput {
    ...
    const route = createSyntheticRoute({
      ...
      slots: {
        'team@dashboard/@team': {
          id: 'slot:team:/',
          name: 'team',
          page: createSyntheticPageModule(() => createElement('div', null, 'team')),
          layoutIndex: 0,
          routeSegments: [],
        },
        'analytics@dashboard/@analytics': {
          id: 'slot:analytics:/',
          name: 'analytics',
          default: createSyntheticPageModule(() => createElement('div', null, 'analytics')),
          layoutIndex: 0,
          routeSegments: null,
        },
    ...
    const result = await buildPageElements(
      createBaseOptions({
        ...
        opts: {
          ...
          interceptPage: createSyntheticPageModule(() => createElement('div', null, 'Intercepted')),
  ...
  it('calls markDynamicUsage when search params have content', async () => {
    function SearchPage(): RenderableOutput {
  ...
  it('does NOT call markDynamicUsage when search params are empty', async () => {
    function NoSearchPage(): RenderableOutput {
      return createElement('div', null, 'No Search')
    }
  ...
  it('passes slot overrides when interception opts have a slot key and page', async () => {
    function InterceptPage(): RenderableOutput {
      return createElement('div', null, 'Intercepted')
    }

    const route = createSyntheticRoute({
      page: createSyntheticPageModule(() => createElement('div', null, 'Main')),
      layouts: [],
      routeSegments: ['feed'],
      pattern: '/feed',
    })
    ...
    const result = await buildPageElements(
      createBaseOptions({
        ...
        opts: {
          ...
          interceptLayouts: [createSyntheticPageModule(() => createElement('div', null, 'Layout'))],
  ...
  it('builds elements for a page that receives search params', async () => {
    function ParamPage(): RenderableOutput {
