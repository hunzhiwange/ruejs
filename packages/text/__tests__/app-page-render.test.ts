...
function captureRecord(value: TextRenderable | AppOutgoingElements): Record<string, unknown> {
...
function createClientReference(id: string): unknown {
  return Object.assign(
    () => {
      throw new Error('client reference should not execute while preparing the RSC payload')
    },
    {
...
function createCommonOptions() {
  ...
  return {
    ...
    options: {
      ...
      element: createElement('div', null, 'page'),
...
describe('form state rendering', () => {
  it('passes action form state to SSR and disables HTML cache writes', async () => {
    ...
    await expect(response.text()).resolves.toBe('<!DOCTYPE html><html>action state</html>')
  })
})
...
describe('app page render lifecycle', () => {
  ...
  it('rerenders HTML responses with the error boundary when SSR reports a recoverable error', async () => {
    const common = createCommonOptions()
    const ssrError = new Error('client page error')
    ...
    const response = await renderAppPageLifecycle({
      ...
      loadSsrHandler: async () => ({
        async handleSsr(
          ...
          _navContext: unknown,
          _fontData: unknown,
          options?: { onSsrError?: (error: unknown) => void },
        ) {
          options?.onSsrError?.(ssrError)
          return createStream(['<html>shell</html>'])
        },
      }),
    })

    expect(common.renderErrorBoundaryResponse).toHaveBeenCalledWith(ssrError)
    await expect(response.text()).resolves.toBe('boundary:client page error')
  })
  ...
  it('passes explicit SSR payload decoders to HTML SSR', async () => {
    const common = createCommonOptions()
    const ssrPayloadDecoder = vi.fn(async () => ({
      [APP_ROOT_LAYOUT_KEY]: '/',
      [AppElementsWire.keys.interceptionContext]: null,
      [AppElementsWire.keys.route]: AppElementsWire.encodeRouteId('/posts/[slug]', null),
      [AppElementsWire.encodePageId('/posts/[slug]', null)]: 'decoded page',
    }))
    const loadSsrHandler = vi.fn(async () => ({
      async handleSsr(
        _rscStream: ReadableStream<Uint8Array>,
        _navContext: unknown,
        _fontData: unknown,
        options?: { ssrPayload?: unknown; ssrPayloadDecoder?: unknown },
      ) {
        expect(options?.ssrPayload).toBeUndefined()
        expect(options?.ssrPayloadDecoder).toBe(ssrPayloadDecoder)
        return createStream(['<html>decoded</html>'])
      },
    }))
    ...
    await expect(response.text()).resolves.toBe('<!DOCTYPE html><html>decoded</html>')
    expect(loadSsrHandler).toHaveBeenCalledTimes(1)
    expect(ssrPayloadDecoder).not.toHaveBeenCalled()
  })
...
describe('layoutFlags injection into RSC payload', () => {
  function createRscOptions(overrides: {
    element?: Record<string, TextRenderable>
    layoutCount?: number
    ...
    const options = {
      ...
      renderToReadableStream(el: TextRenderable | AppOutgoingElements) {
  ...
  it('preserves client references in prepared RSC payloads even when SSR can resolve them', async () => {
    const clientReference = createClientReference('/components/LikeButton.tsx#default')
    const pageId = AppElementsWire.encodePageId('/test', null)
    function ResolvedLikeButton() {
      return null
    }
  ...
  it('injects concrete artifact compatibility metadata from the render boundary', async () => {
    const originalBuildId = process.env.__TEXT_BUILD_ID
    process.env.__TEXT_BUILD_ID = 'deploy-test'
    const { options, getCapturedElement } = createRscOptions({
      element: {
    ...
    try {
      ...
      if (originalBuildId === undefined) {
        ...
        process.env.__TEXT_BUILD_ID = originalBuildId
      }
    }
...
describe('dev error reporting for invalid dynamic usage — issue #1195', () => {
  it('does not double-log an uncaught error that Rue already stamped with a digest', async () => {
    const common = createCommonOptions()
  ...
  it('logs a caught error that has no digest (Rue never saw it)', async () => {
    const common = createCommonOptions()
    const consoleErrorSpy = vi.spyOn(console, 'error').mockImplementation(() => {})
