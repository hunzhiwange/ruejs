...
import { generateRscEntry } from '../src/entries/app-rsc-entry.js'
import { generateSsrEntry } from '../src/entries/app-ssr-entry.js'
import type { AppRoute } from '../src/routing/app-router.js'
import text from '../src/index.js'
import {
  APP_FIXTURE_DIR,
  buildIsolatedAppFixture,
...
function isRueCompatRscRuntimeOptimizeInclude(entry: string): boolean {
  const ruePackage = 'rue'
  const rueDomPackage = `${ruePackage}-dom`

  return (
    entry === ruePackage ||
    entry === rueDomPackage ||
    entry === `${ruePackage}/jsx-runtime` ||
    entry === `${ruePackage}/jsx-dev-runtime` ||
    entry === `${rueDomPackage}/client` ||
    entry === `${rueDomPackage}/server` ||
    entry === `${rueDomPackage}/server.edge` ||
    entry === `${rueDomPackage}/static.edge`
  )
}
...
describe('App Router integration', () => {
  ...
  beforeAll(async () => {
    ;({ server, baseUrl } = await startFixtureServer(APP_FIXTURE_DIR, {
      appRouter: true,
      textOptions: { experimental: { clientReferenceDedup: true } },
    }))
  }, 30000)
  ...
  // Ported from Text.js: test/e2e/prerender.test.ts
  ...
  // Ported from Text.js: test/e2e/app-dir/cache-components/cache-components.params.test.ts
  ...
  it('renders pages with params named then, catch, finally, and status', async () => {
    const { res, html } = await fetchHtml(
      baseUrl,
      '/params-shadow/foo/bar/baz/qux?then=qfoo&catch=qbar&finally=quux&status=qok',
    )
    expect(res.status).toBe(200)
    ...
    expect(textContentByTestId(html, 'search-then')).toBe('qfoo')
    expect(textContentByTestId(html, 'search-catch')).toBe('qbar')
    expect(textContentByTestId(html, 'search-finally')).toBe('quux')
    expect(textContentByTestId(html, 'search-status')).toBe('qok')
    expect(textContentByTestId(html, 'search-is-thenable')).toBe('yes')
  })
  ...
  // Text.js sets the RSC response Content-Type to exactly "text/x-component"
  // (no charset). Several Text.js tests use strict equality:
  //   .textjs-ref/test/e2e/app-dir/app/index.test.ts L362, L371
  //   .textjs-ref/test/e2e/app-dir/segment-cache/deployment-skew/deployment-skew.test.ts L80
  // Source constant:
  //   .textjs-ref/packages/text/src/client/components/app-router-headers.ts L17
  ...
  it('uses text/x-component for the RSC Content-Type with no charset suffix', async () => {
    ...
    // Drain the RSC body so the server element runtime installed for the
    // stream can restore before later tests create new App Router servers.
  ...
  it('returns RSC stream for .rsc requests', async () => {
    ...
    // RSC stream should contain serialized server component payload data.
  ...
  // causing Rue context providers to be duplicated (createContext runs twice).
  ...
  it('SSR renders a real app route that calls useRouter()', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/hooks-router')
    expect(res.status).toBe(200)
  ...
  it('renders nested parallel route from layout-only parent', async () => {
    // Ported from Text.js: test/e2e/app-dir/parallel-routes-and-interception/parallel-routes-and-interception.test.ts (line 510)
  ...
  // Ported from Text.js: test/e2e/app-dir/parallel-routes-catchall-groups/parallel-routes-catchall-groups.test.ts
  ...
  // Ported from Text.js: test/e2e/app-dir/parallel-routes-use-selected-layout-segment
  ...
  it("useSelectedLayoutSegment('team') returns the leaf segment for nested slot routes", async () => {
    // Mirrors Text.js @auth/reset/withEmail coverage:
  ...
  it('renders intercepted photo modal on RSC navigation from feed', async () => {
    ...
    const res = await fetch(`${baseUrl}/photos/42.rsc`, {
      headers: {
        ...
        'X-Text-Interception-Context': '/feed',
      },
    })
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/x-component')
  ...
  it('extracts actual URL params for intercepted routes with dynamic source routes', async () => {
    ...
    //
    // The X-Text-Interception-Context header carries the source pathname
    // (the equivalent of Text.js' Text-URL header). Without it the matcher
    // must NOT fire the interception, matching Text.js' rewrite semantics —
    ...
    const res = await fetch(`${baseUrl}/team/42/settings.rsc`, {
      headers: {
        Accept: 'text/x-component',
        'X-Text-Interception-Context': '/team/42/members',
      },
    })
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/x-component')
  ...
  it('does NOT fire intercept on direct RSC request without interception context', async () => {
    // Mirrors Text.js: interception rewrites only fire when the Text-URL
    // header matches the intercepting-route regex. A direct `.rsc` fetch
    // with no source pathname must render the underlying page.
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/generate-interception-routes-rewrites.ts
    const res = await fetch(`${baseUrl}/team/42/settings.rsc`, {
      headers: { Accept: 'text/x-component' },
    })
    expect(res.status).toBe(200)
  ...
  it('returns Method Not Allowed for unsupported HTTP methods on route handlers', async () => {
    ...
    // Text.js does not emit an Allow header on 405 responses
  ...
  it('rejects middleware control responses returned from route handlers', async () => {
    // The TextResponse.text() case is ported from Text.js:
    ...
    // The TextResponse.rewrite() case mirrors the adjacent App Route module validation.
    const textRes = await fetch(`${baseUrl}/api/invalid-text-response-text`)
    expect(textRes.status).toBe(500)
    expect(await textRes.text()).toBe('')

    const rewriteRes = await fetch(`${baseUrl}/api/invalid-text-response-rewrite`)
  ...
  it('notFound() from async page with loading.tsx returns 404 (TEXT_NOT_FOUND digest)', async () => {
    ...
    // "TEXT_NOT_FOUND" digest (not "TEXT_HTTP_ERROR_FALLBACK;404"), which
  ...
  it('forbidden() from async page with loading.tsx returns 403 (digest status preserved)', async () => {
    ...
    // status code from TEXT_HTTP_ERROR_FALLBACK;403 rather than coercing
  ...
  it('unauthorized() from async page with loading.tsx returns 401 (digest status preserved)', async () => {
    ...
    // verifies the post-shell digest swap honors TEXT_HTTP_ERROR_FALLBACK;401
  ...
  it('forbidden() thrown from a layout uses the forbidden boundary', async () => {
    // Ported from Text.js: test/e2e/app-dir/forbidden/basic/forbidden-basic.test.ts
  ...
  it('unauthorized() thrown from a layout uses the unauthorized boundary', async () => {
    // Ported from Text.js: test/e2e/app-dir/unauthorized/basic/unauthorized-basic.test.ts
  ...
  // When a Server Component imports a client-only hook from text/navigation
  // without the "use client" directive, text should surface a clear error
  ...
  it("errors when Rue client hook is used in a Server Component without 'use client' (#834)", async () => {
    const { res, html } = await fetchHtml(baseUrl, '/missing-use-client-rue-hook')
    expect(res.status).toBe(200) // error boundary renders, not a 500
    // The error message should be clear and actionable
    ...
    expect(html).toContain('use client')
    // Should NOT contain the actual page content (it errored before rendering)
    expect(html).not.toContain('Missing use client rue hook test')
  })
  ...
  // Ported from Text.js: test/e2e/app-dir/rsc-redirect/rsc-redirect.test.ts
  ...
  // navigation request, Text.js returns HTTP 200 with the redirect instruction
  // encoded in the RSC payload. The status code must be 200 because the
  // client uses `redirect: 'manual'` fetch semantics when validating
  // cache-busting; a raw 307 would break that flow. text addresses RSC
  ...
  it('redirect() from Server Component returns 200 + RSC payload for RSC navigations', async () => {
    ...
    expect(res.headers.get('content-type')).toContain('text/x-component')
    const text = await res.text()
    // The redirect must be embedded in the RSC payload so the client router
    // can detect it and navigate. Match Text.js's encoding: the TEXT_REDIRECT
    // digest is serialized as part of the Rue error chunk.
    expect(text).toContain('TEXT_REDIRECT')
    expect(text).toContain('/about')
  })

  it('permanentRedirect() from Server Component returns 200 + RSC payload for RSC navigations', async () => {
    ...
    // permanent redirect. Mirrors Text.js's `redirect()` vs
    // `permanentRedirect()` distinction in the RSC payload encoding.
  ...
  // throws redirect(), Text.js still returns 200 — metadata is suspended in
  // SSR so the redirect rides inside the streamed RSC payload rather than
  // becoming an HTTP-level 307. See:
  //   https://github.com/cloudflare/vinext/issues/1347
  it('redirect() from generateMetadata returns 200 with redirect payload (RSC)', async () => {
  ...
  it('redirect() from generateMetadata returns 200 for SSR document request', async () => {
    ...
    // RSC payload, not as an HTTP-level redirect.
    expect(res.status).toBe(200)
    const text = await res.text()
    expect(text).toContain('TEXT_REDIRECT')
    expect(text).toContain('/about')
  ...
  // ── probePage() with Text.js 15+ async params/searchParams ──
  ...
  it('redirect() from async page with loading.tsx returns 307 (digest captured during shell render)', async () => {
    ...
    // redirect throw, and Rue would serialize a "Switched to client
    ...
    // the rscErrorTracker captures the TEXT_REDIRECT digest from Rue's
  ...
  it('permanentRedirect() from async page with loading.tsx returns 308 (digest status preserved)', async () => {
    ...
    // the TEXT_REDIRECT digest (308) rather than coercing to the 307
    // default.
  ...
  it('redirect() inside Suspense boundary preserves digest in RSC payload', async () => {
    ...
    // during RSC streaming. The onError callback preserves the TEXT_REDIRECT
    ...
    expect(html).toContain('TEXT_REDIRECT')
    expect(html).toContain('/about')
  })
  ...
  it('notFound() inside Suspense boundary preserves digest for not-found UI', async () => {
    ...
    expect(html).toMatch(
      /(\$RX\("[^"]*","TEXT_HTTP_ERROR_FALLBACK|data-dgst="TEXT_HTTP_ERROR_FALLBACK;404"|\\"digest\\":\\"TEXT_HTTP_ERROR_FALLBACK;404\\")/,
    )
  })

  it('async server throw in Suspense avoids compat dev decode crash', async () => {
    // Regression for issue #50:
    // Compat dev-mode RSC payload decoding can crash in resolveErrorDev() with
    ...
    // after the route boundary wins, Rue can emit either the route error
    // boundary or its Suspense client-render marker. The key regression check
    // is that decoding no longer crashes with a null dispatcher while consuming
    // that error chunk.
    expect(html).toMatch(/data-testid="rue19-dev-rsc-error-boundary"|Switched to client rendering/)
    expect(html).toContain('rue19-dev-rsc-error')
  ...
  it('layout generateMetadata() does not receive searchParams (Text.js parity)', async () => {
    // Parity test: In Text.js, layout generateMetadata() does NOT receive
  ...
  it('renders static metadata (export const metadata) as head elements', async () => {
    ...
    // Keywords meta tag
    expect(html).toMatch(/name="keywords".*content="test,metadata,text"/)
    // Open Graph tags
  ...
  it('RSC stream for metadata-test page includes metadata head tags', async () => {
    // The .rsc endpoint returns the RSC payload.
    ...
    const rscText = await res.text()
    // The RSC stream contains serialized Rue elements, including title and meta
  ...
  it('serves /icon from dynamic icon.tsx using ImageResponse', async () => {
    // This test verifies the full pipeline: icon.tsx → text/og → satori → resvg → PNG
  ...
  it("export const dynamic = 'force-static' sets long-lived Cache-Control", async () => {
    ...
    expect(res.headers.get('x-text-cache')).toBe('STATIC')
  })
  ...
  it('sets optimizeDeps.entries for rsc, ssr, and client environments so deps are discovered at startup', () => {
    ...
    expect(rscGlob).toMatch(/app\/\*\*\/\*\.\{tsx,ts,jsx\}/)
    expect(ssrGlob).toMatch(/app\/\*\*\/\*\.\{tsx,ts,jsx\}/)
    expect(clientGlob).toMatch(/app\/\*\*\/\*\.\{tsx,ts,jsx\}/)
  ...
  it('pre-includes RSC compat dependencies in optimizeDeps.include to avoid late discovery', () => {
    // Compat deps that are imported by virtual modules (not user code)
    ...
    const ssrRuntimeEntries = RSC_RUE_SSR_EXTERNAL_ENTRIES.filter(
      entry => entry !== RSC_RUE_SERVER_DOM_CLIENT_EDGE,
    )
    for (const entry of ssrRuntimeEntries) {
      expect(ssrInclude, `ssr compat include should contain ${entry}`).toContain(entry)
    }
  ...
  it('keeps Rue runtime entries out of the Rue-native server optimizer includes', () => {
    const ssrIncludes = (server.config.environments.ssr?.optimizeDeps?.include ?? []).filter(
      (entry): entry is string => typeof entry === 'string',
    )
    const rscIncludes = (server.config.environments.rsc?.optimizeDeps?.include ?? []).filter(
      (entry): entry is string => typeof entry === 'string',
    )

    expect(ssrIncludes.filter(isRueCompatRscRuntimeOptimizeInclude)).toEqual([])
    expect(rscIncludes.filter(isRueCompatRscRuntimeOptimizeInclude)).toEqual([])
  })
  ...
  it('allows server action POST with matching Origin header', async () => {
    // Ported from Text.js: test/e2e/app-dir/no-server-actions/no-server-actions.test.ts
    ...
    expect(res.status).toBe(404)
    expect(res.headers.get('x-textjs-action-not-found')).toBe('1')
...
describe('App Router Production build', () => {
  ...
  beforeAll(async () => {
    fixtureDir = await createIsolatedFixture(APP_FIXTURE_DIR, 'text-app-prod-build-')
    outDir = path.resolve(fixtureDir, 'dist')
  ...
  it('produces RSC/SSR/client bundles via vite build', async () => {
    ...
    // Client should have hashed JS assets under Text.js's canonical
    // `_text/static/` directory (matches `resolveAssetsDir("")`).
    const clientAssets = fs.readdirSync(path.join(outDir, 'client', '_text', 'static'))
  ...
  it('serves production build via preview server', async () => {
    ...
    try {
      ...
      // tag (via Rue's bootstrapModules option) referencing hashed assets.
      expect(homeHtml).toMatch(/<script[^>]+type="module"[^>]+src="\/_text\/static\/[^"]+\.js"/)
      ...
      await drainResponseBody(rscRes)
    } finally {
...
describe('App Router Production server (startProdServer)', () => {
  ...
  beforeAll(async () => {
    fixtureDir = await createIsolatedFixture(APP_FIXTURE_DIR, 'text-app-prod-server-')
    outDir = path.resolve(fixtureDir, 'dist')

    // Build the isolated app-basic fixture to its default dist/ directory.
    const builder = await createBuilder({
      root: fixtureDir,
      configFile: false,
      plugins: [text({ appDir: fixtureDir })],
  ...
  it('does not reuse cached HTML across requests with different CSP nonces', async () => {
    ...
    expect(firstHtml).toContain(
      '<script nonce="first">Object.assign(((self[Symbol.for("text.navigationRuntime")]',
    )
    ...
    expect(secondHtml).toContain(
      '<script nonce="second">Object.assign(((self[Symbol.for("text.navigationRuntime")]',
    )
  ...
  it('serves Pages Router edge API ImageResponse routes in hybrid production', async () => {
    // Ported from Text.js: test/e2e/og-api/index.test.ts
  ...
  // header) returns 200 with an RSC payload — that path is covered by the
  ...
  it('serves public files from the build output', async () => {
    // Ported from Text.js: test/production/export/index.test.ts
  ...
  it('serves public files under basePath and 404s without it', async () => {
    // Ported from Text.js: test/e2e/basepath/basepath.test.ts
    // https://github.com/vercel/next.js/blob/canary/test/e2e/basepath/basepath.test.ts
    const tmpDir = mkdtempInTextPackage('text-app-public-basepath-')
    ...
    try {
      ...
      const textConfig = fs.readFileSync(textConfigPath, 'utf-8')
      fs.writeFileSync(
        textConfigPath,
        textConfig.replace(
  ...
  it('revalidateTag invalidates App Router ISR page entries by fetch tag', async () => {
    ...
    expect(res3.headers.get('x-text-cache')).toBe('MISS')
  })
  ...
  it('page ISR + searchParams: RSC requests stay dynamic instead of serving cached query data', async () => {
    ...
    expect(res1.headers.get('x-text-cache')).toBeNull()
    ...
    expect(res2.status).toBe(200)
    expect(res2.headers.get('x-text-cache')).toBeNull()
  ...
  it('route handler ISR: first GET returns MISS', async () => {
    ...
    expect(res.status).toBe(200)
    expect(res.headers.get('x-text-cache')).toBe('MISS')
  })
  ...
  it('route handler ISR: STALE serves stale data and triggers background regen', async () => {
    ...
    } while (freshRes.headers.get('x-text-cache') !== 'HIT' && Date.now() < deadline)

    // HIT — fresh data from background regen
    expect(freshRes.headers.get('x-text-cache')).toBe('HIT')
    expect(freshBody.timestamp).not.toBe(cachedTimestamp) // New data
  })

  // Test pattern ported from Text.js:
  ...
  it('route handler ISR: STALE completes quickly without blocking on background regen', async () => {
    ...
    const staleDuration = Date.now() - staleStart
    expect(stale.headers.get('x-text-cache')).toBe('STALE')
  ...
  it('route handler ISR: auto-HEAD returns cached headers with empty body', async () => {
    ...
    await getRes.text()
    const cacheHeader = getRes.headers.get('x-text-cache')
    ...
    expect(headRes.headers.get('x-text-cache')).toBe('HIT')
  ...
  it('middleware request header overrides still apply after middleware calls headers() first', async () => {
    ...
    // `headers()` *before* returning `TextResponse.text({ request: { headers } })`
    ...
    //
    // Discovered with @clerk/textjs, whose `clerkClient()` calls
...
describe('App Router Production server worker entry compatibility', () => {
  it('accepts Worker-style default exports from dist/server/index.js', async () => {
    const outDir = mkdtempInTextPackage('text-prod-worker-entry-')
...
`,
  ...
  it('reports a clear error for unsupported app router entry shapes', async () => {
    const outDir = mkdtempInTextPackage('text-prod-worker-invalid-')
    const serverDir = path.join(outDir, 'server')
    fs.mkdirSync(serverDir, { recursive: true })
    fs.writeFileSync(path.join(outDir, 'package.json'), JSON.stringify({ type: 'module' }))
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        '[text] App Router entry must export either a default handler function or a Worker-style default export with fetch()',
      )
    } finally {
...
describe('App Router Production server self-hosted text/font/google headers', () => {
  // Regression for a bug where text's `text/font/google` self-hosting
  ...
  // tags, and the `<style data-text-fonts>` `@font-face src: url(...)`
  // block. `fetchAndCacheFont` in `packages/text/src/plugins/fonts.ts`
  // downloaded Google Fonts `.woff2` files into `<root>/.text/fonts/`
  ...
  // 404s (`<origin>/home/user/project/.text/fonts/...`) on every
  // request and fell back to the real font only via the browser's
  // unrelated runtime retry of the stylesheet CDN.
  //
  // The fix uses a separate fixture (`__tests__/fixtures/font-google-multiple`)
  // rather than `app-basic` because `app-basic` is shared by many other
  // tests — adding `text/font/google` to its root layout would force a
  ...
  const fontCacheDir = path.resolve(FONT_FIXTURE_DIR, '.text')
  ...
  beforeAll(async () => {
    ...
    // so `text` resolves as a workspace package during the in-process
    // build below.
    ...
    globalThis.fetch = async (input: unknown, init?: RequestInit) => {
      ...
      if (url.includes('fonts.gstatic.com')) {
        ...
        // under `.text/fonts/<family>/<hash>.woff2`. The test never reads
    ...
    try {
      const builder = await createBuilder({
        ...
        configFile: false,
        plugins: [text({ appDir: FONT_FIXTURE_DIR })],
        logLevel: 'silent',
      })
      await builder.buildApp()
    } finally {
  ...
  it('emits served URLs in the HTTP Link response header (not filesystem paths)', async () => {
    ...
    // `</home/user/project/.text/fonts/geist-<hash>/geist-<hash>.woff2>`.
    expect(link).toContain('/_text/static/_text_fonts/')
    ...
    expect(link).not.toContain('.text/fonts')
  })
  ...
  it('emits served URLs in the body <link rel=preload> tags', async () => {
    ...
    expect(html).not.toContain('.text/fonts')
  })

  it('emits served URLs in the injected <style data-text-fonts> block', async () => {
    ...
    expect(styleContent).toMatch(/url\(\/_text\/static\/_text_fonts\/[^)]+\.woff2\)/)
    expect(styleContent).not.toContain(FONT_FIXTURE_DIR)
    expect(styleContent).not.toContain('.text/fonts')
  })
...
describe('App Router Static export', () => {
  ...
  beforeAll(async () => {
    ;({ fixtureDir, rscBundlePath } = await buildIsolatedAppFixture(
      APP_FIXTURE_DIR,
      'text-app-static-export-',
    ))
    exportDir = path.resolve(fixtureDir, 'out')
  }, 120_000)
  ...
  it('reports errors for dynamic routes without generateStaticParams', async () => {
    ...
    const fakeRoutes = [
      {
        pattern: '/fake/:id',
        pagePath: path.resolve(fixtureDir, 'app', 'page.tsx'),
        routePath: null,
        layouts: [],
        templates: [],
        parallelSlots: [],
    ...
    const config = await resolveTextConfig({ output: 'export' })
    const tempDir = path.resolve(fixtureDir, 'out-temp-app')
  ...
  it('skips route handlers with warning', async () => {
    ...
    const fakeRoutes = [
      {
        pattern: '/api/test',
        pagePath: null,
        routePath: path.resolve(fixtureDir, 'app', 'api', 'hello', 'route.ts'),
        layouts: [],
        templates: [],
        parallelSlots: [],
    ...
    const config = await resolveTextConfig({ output: 'export' })
    const tempDir = path.resolve(fixtureDir, 'out-temp-api')
...
describe('metadata routes integration (App Router)', () => {
  ...
  it('serves sitemap routes that import but do not render client references', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-dynamic-routes/index.test.ts
  ...
  it('injects file-based metadata into head tags for static metadata files', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-static-file/metadata-static-file-static-route.test.ts
  ...
  it('injects sizes=any for static SVG icon metadata routes', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-svg-icon/metadata-svg-icon.test.ts
  ...
  it('injects dynamic metadata image routes into the head', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata/metadata.test.ts
...
describe('App Router text.config.js features (dev server integration)', () => {
  let server: ViteDevServer
  let baseUrl: string

  beforeAll(async () => {
    // Uses the permanent text.config.ts in the app-basic fixture.
  ...
  it('applies redirects from text.config.js (permanent)', async () => {
  ...
  it('applies custom headers from text.config.js on API routes', async () => {
    const res = await fetch(`${baseUrl}/api/hello`)
    expect(res.headers.get('x-custom-header')).toBe('text-app')
  })

  it('applies custom headers from text.config.js on page routes', async () => {
    const res = await fetch(`${baseUrl}/about`)
...
describe('App Router text.config.js features (generateRscEntry)', () => {
  ...
  it('generates custom header handling code when headers are provided', () => {
    const code = generateRscEntry('/tmp/test/app', minimalRoutes, null, [], null, '', false, {
      headers: [{ source: '/api/(.*)', headers: [{ key: 'X-Custom-Header', value: 'text' }] }],
    })
  ...
  it('routes hybrid Pages API misses through the Pages server entry', () => {
    ...
    expect(code).toContain('if (__pagesPathname.startsWith("/api/") || __pagesPathname === "/api")')
  ...
  it('loads allowedDevOrigins from text.config into the virtual RSC entry', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-app-rsc-allowed-dev-origins-'))
    try {
  ...
  experimental: {
    serverActions: {
      ...
      const testServer = await createServer({
        root: tmpDir,
        configFile: false,
        plugins: [text({ appDir: tmpDir })],
        server: { port: 0 },
        logLevel: 'silent',
      })

      try {
        const resolved = await testServer.pluginContainer.resolveId('virtual:text-rsc-entry')
...
describe('App Router middleware with TextRequest', () => {
  let server: ViteDevServer
  let baseUrl: string
  ...
  it('middleware receives TextRequest and can use .textUrl', async () => {
    // The middleware sets x-mw-pathname from request.textUrl.pathname
  ...
  it('middleware TextRequest.textUrl.pathname strips .rsc suffix', async () => {
  ...
  it('middleware receives TextRequest and can use .cookies', async () => {
    // The middleware checks request.cookies.get() which requires TextRequest
    const res = await fetch(`${baseUrl}/about`, {
      headers: {
  ...
  it('middleware can redirect using TextRequest', async () => {
  ...
  it('does not leak x-middleware-text or x-middleware-rewrite headers to the client', async () => {
    // TextResponse.text() sets x-middleware-text internally.
    ...
    const textRes = await fetch(`${baseUrl}/about`)
    expect(textRes.status).toBe(200)
    // Middleware ran (verified by the custom header it sets)
    expect(textRes.headers.get('x-mw-ran')).toBe('true')
    // Internal headers must NOT be present
    expect(textRes.headers.get('x-middleware-text')).toBeNull()
    expect(textRes.headers.get('x-middleware-rewrite')).toBeNull()
    // Check that no x-middleware-* header leaked at all
    for (const [key] of textRes.headers) {
      expect(key.startsWith('x-middleware-')).toBe(false)
    }

    // TextResponse.rewrite() sets x-middleware-rewrite internally.
    ...
    expect(rewriteRes.headers.get('x-middleware-text')).toBeNull()
...
describe('RSC preload hint normalization', () => {
  it('generateRscEntry delegates renderToReadableStream hint normalization', () => {
    ...
    // rewrites RSC HL hint "stylesheet" → "style" at the stream source,
    ...
    const countOccurrences = (needle: string) => code.split(needle).length - 1

    expect(code).toContain('_renderAppRscPayloadToReadableStream')
    expect(code).toContain('createRscRenderer')
    expect(code).toContain(
      'const appRscRendererAdapter = createTextRscRendererAdapter(\n  createRscRenderer(_renderAppRscPayloadToReadableStream),\n);',
    )
    expect(code).toContain(
      'const renderToReadableStream = appRscRendererAdapter.renderToReadableStream;',
    )
    expect(countOccurrences('import { createTextRscRendererAdapter }')).toBe(1)
    expect(countOccurrences('createTextRscRendererAdapter')).toBe(2)
    expect(countOccurrences('createTextRscRendererAdapter(')).toBe(1)
    expect(countOccurrences('createRscRenderer(_renderAppRscPayloadToReadableStream)')).toBe(1)
  })
})
// ── Client reference preloading (Issue #256) ─────────────────────────────────
//
// On the first SSR request after server start, client reference modules are
// loaded lazily via async import(). The memoize cache in @rue-js/rsc is
// cold, so __vite_rsc_client_require__ returns an unresolved Promise. Without
// <Suspense> wrapping the root shell, Rue SSR rejects and the server returns
...
describe('Client reference preloading (Issue #256)', () => {
  it('preloading correctly warms the memoize cache', async () => {
    // Replicate the memoize + lazy-load pattern from @rue-js/rsc
    ...
    // already settled. Rue SSR calls __vite_rsc_client_require__ and
...
// ── Auto-registration of @rue-js/rsc ────────────────────────────────────────
...
describe('RSC plugin auto-registration', () => {
  ...
  beforeAll(async () => {
    // Create a server with ONLY text() — no explicit @rue-js/rsc.
    ...
    cacheDir = mkdtempInTextPackage('text-auto-rsc-cache-')
    server = await createServer({
      root: APP_FIXTURE_DIR,
      cacheDir,
      configFile: false,
      plugins: [text({ appDir: APP_FIXTURE_DIR })],
  ...
  it('does not double-register when RSC plugin is already present', async () => {
    const { createServer } = await import('vite')
    const rsc = (await import('@rue-js/rsc')).default
    const explicitCacheDir = mkdtempInTextPackage('text-explicit-rsc-cache-')

    // Create a server with BOTH text({ rsc: false }) and explicit rsc().
    ...
    const serverWithExplicitRsc = await createServer({
      ...
      configFile: false,
      plugins: [text({ appDir: APP_FIXTURE_DIR, rsc: false }), rsc({ entries: RSC_ENTRIES })],
      optimizeDeps: { holdUntilCrawlEnd: true },
      server: { port: 0, cors: false },
      logLevel: 'silent',
    })
    ...
    try {
      ...
      expect(addr && typeof addr === 'object' ? addr.port : null).toBeGreaterThan(0)
    } finally {
      try {
        await serverWithExplicitRsc.close()
      } finally {
        fs.rmSync(explicitCacheDir, { recursive: true, force: true })
      }
    }
  }, 30000)
  ...
  it('throws an error when user double-registers rsc() alongside auto-registration', async () => {
    ...
    await expect(
      createBuilder({
        root: APP_FIXTURE_DIR,
        configFile: false,
        plugins: [text({ appDir: APP_FIXTURE_DIR }), rsc({ entries: RSC_ENTRIES })],
        logLevel: 'silent',
      }),
    ).rejects.toThrow('Duplicate @rue-js/rsc detected')
  }, 30000)
  ...
  it('auto-injects RSC plugin when src/app exists but root-level app/ does not', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-src-app-'))
    try {
      ...
      // Symlink node_modules so createRequire can find @rue-js/rsc
      ...
      const plugins = text({ appDir: tmpDir, rue: false })
  ...
  it('does NOT auto-inject RSC plugin when neither app/ nor src/app/ exists', async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-no-app-'))
    try {
...
// must forward credential headers like Text.js while still stripping
...
describe('App Router external rewrite proxy credential forwarding', () => {
  ...
  beforeAll(async () => {
    ...
    // 2. Set env var so the app-basic text.config.ts adds the external rewrite
    ...
    // 3. Start the App Router dev server (reads text.config.ts at boot)
    ;({ server, baseUrl } = await startFixtureServer(APP_FIXTURE_DIR, { appRouter: true }))
  }, 30000)
  ...
  it('forwards credential headers and strips x-middleware-* headers from proxied requests to external rewrite targets', async () => {
    ...
    expect(capturedHeaders!['x-middleware-text']).toBeUndefined()
    expect(capturedHeaders!['x-text-prerender-secret']).toBeUndefined()
  ...
  it('proxies external URLs returned by middleware rewrites with body and headers', async () => {
    ...
    expect(capturedHeaders!['x-text-mw-ctx']).toBeUndefined()
  })
...
describe('generateRscEntry ISR code generation', () => {
  ...
  it('root params runtime getter returns current request values', async () => {
    const { getRootParam, pickRootParams, setRootParams } =
      await import('../src/shims/root-params.js')
