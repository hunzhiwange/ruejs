/**
 * Regression tests: RSC lazy stream context-clear bug + __TEXT_RSC_NAV__ hydration embedding
 *
 ...
 * TextIntlClientProviderServer calling getMessages()) sees headersContext = null
 ...
 *
 * Fixture: __tests__/fixtures/app-basic/app/textjs-compat/rsc-context-lazy-stream/
 ...
 * and asserts the sentinel value appears in the RSC payload response, proving
 * the layout's headers() call ran successfully during lazy stream consumption.
 *
 * ── Bug 2: __TEXT_RSC_NAV__ missing from HTML (client hydration mismatch) ─
 *
 ...
 * a client/server hydration mismatch whenever any "use client" component
 * called usePathname() or useSearchParams().
 *
 * Fix: generateSsrEntry() embeds __TEXT_RSC_NAV__ = { pathname, searchParams }
 * (where searchParams is an array of [key, value] pairs to preserve duplicates)
 * and __TEXT_RSC_PARAMS__ = { ...params } as <script> tags in <head>.
 * generateBrowserEntry() reads self.__TEXT_RSC_NAV__ before hydrateRoot()
 * and calls setNavigationContext() so the client snapshot matches the server.
 *
 * The tests verify:
 *   1. The HTML <head> contains the __TEXT_RSC_NAV__ script tag
 *   2. The embedded pathname matches the actual request path
 *   3. The embedded searchParams matches the actual query string
 *   4. __TEXT_RSC_PARAMS__ carries dynamic segment values
 *   5. The SSR-rendered values from usePathname()/useSearchParams() agree with
 *      the __TEXT_RSC_NAV__ payload — ensuring getServerSnapshot will match
 *      the HTML Rue tries to hydrate
 *
 * Fixture: __tests__/fixtures/app-basic/app/textjs-compat/nav-context-hydration/
 *   page.tsx   — RSC page rendering <NavInfo /> (a "use client" component)
 *   nav-info.tsx — "use client": renders usePathname(), useSearchParams()
 *   [id]/page.tsx — dynamic-segment variant for __TEXT_RSC_PARAMS__ testing
 */
...
beforeAll(async () => {
  ...
  await Promise.all([
    fetch(`${_baseUrl}/textjs-compat/rsc-context-lazy-stream.rsc`, {
      headers: {
...
const LAZY_ROUTE = '/textjs-compat/rsc-context-lazy-stream'
...
describe('RSC lazy stream: headers() context survives until stream is consumed', () => {
  ...
  //   2. page.tsx calls notFound() → throws TEXT_NOT_FOUND digest
  //   3. text catches it → calls renderHTTPAccessFallbackPage(route, 404, true, ...)
  ...
  // correct context and SENTINEL appears in the RSC payload response.
  ...
  it('RSC request: layout headers() context survives lazy stream consumption (regression)', async () => {
    ...
    // In the RSC payload format this appears as a prop in the serialized tree.
    // If the early-clear bug is present, the layout sees null context and
    // data-request-id is "missing" — SENTINEL is absent from the RSC body.
...
// ── Suite 2: __TEXT_RSC_NAV__ hydration embedding ──────────────────────────
//
// These tests verify the server embeds a correct __TEXT_RSC_NAV__ payload so
...
const NAV_ROUTE = '/textjs-compat/nav-context-hydration'
const RSC_BOOTSTRAP_PREFIX =
  'Object.assign(((self[Symbol.for("text.navigationRuntime")]??={bootstrap:{routeManifest:null},functions:{}}).bootstrap.rsc??={rsc:[]}),{params:'
...
describe('navigation runtime RSC bootstrap: nav context embedded for hydration snapshot consistency', () => {
  ...
  it('navigation runtime pathname agrees with SSR-rendered usePathname() output', async () => {
    ...
    // __TEXT_RSC_NAV__). Both must agree — if getServerSnapshot returns the
    // embedded pathname and it matches the SSR-rendered value, Rue won't detect
    // a mismatch.
  ...
  it('SSR-rendered useSearchParams() reflects query params (confirms parity source)', async () => {
    ...
    // The __TEXT_RSC_NAV__ payload comes from the same navContext.
  ...
  // ── 6. __TEXT_RSC_PARAMS__ for dynamic segment routes ──────────────────
  //
  // When the route has a dynamic segment (e.g. /nav-context-hydration/hello),
  // __TEXT_RSC_PARAMS__ must carry { id: "hello" }.
  // The browser entry reads self.__TEXT_RSC_PARAMS__ and passes it as
  ...
  //
  // Both __TEXT_RSC_NAV__ and __TEXT_RSC_PARAMS__ are injected into <head>
  ...
  // __TEXT_RSC_NAV__, not the raw URL including query string.
