/**
 * Text.js Compatibility Tests: metadata
 *
 ...
 *
 * NOTE: Most Text.js metadata tests use Playwright (browser.eval('document.title')).
 * We test at the SSR level by checking HTML <head> content directly.
 *
 * Fixture pages live in:
 * - fixtures/app-basic/app/textjs-compat/metadata-*
 */
...
describe('Text.js compat: metadata', () => {
  ...
  it('should render title in <head>', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-title')
  ...
  it('should apply title template from layout', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-title-template')
  ...
  // Text.js: 'should support stashed title in one layer'
  ...
  it('should apply title template to child page', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-title-template/child')
    // Layout template "%s | Layout", child page title "Extra Page"
    expect(html).toContain('<title>Extra Page | Layout</title>')
  })

  // ── Basic metadata tags ──────────────────────────────────────
  // Text.js: 'should support other basic tags'
  ...
  it('should render generator meta tag', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-basic')
    expect(html).toMatch(/meta\s+name="generator"\s+content="text\.js"/)
  })
  ...
  it('should render keywords meta tag', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-basic')
    // Text.js joins keywords with "," (no space) — match that exactly
    expect(html).toMatch(/meta\s+name="keywords"\s+content="text\.js,rue,javascript"/)
  })
  ...
  it('should render og:title', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-opengraph')
  ...
  // Text.js allows openGraph.images to be a single object (not wrapped in an array).
  // See: https://textjs.org/docs/app/api-reference/functions/generate-metadata#opengraph

  it('should render og:image when images is a single object', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-opengraph-single-image')
    expect(html).toMatch(
  ...
  // ── Twitter ──────────────────────────────────────────────────
  // Text.js: 'should support twitter card summary_large_image'
  ...
  it('should render twitter:card', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-twitter')
  ...
  // ── Robots (complex) ────────────────────────────────────────
  // Text.js: 'should support robots tags'
  ...
  it('should render complex robots meta tag', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-robots')
  ...
  // ── Alternates ───────────────────────────────────────────────
  // Text.js: 'should support alternate tags'
  ...
  it('should render canonical link', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-alternates')
  ...
  it('should render hreflang alternate links', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-alternates')
    // Rue JSX renders as hrefLang (camelCase) in HTML output
  ...
  //
  // Text.js passes a `parent` Promise<ResolvedMetadata> as the second argument
  ...
  it('should pass parent metadata to generateMetadata via parent parameter', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-parent-generate')
  ...
  it('parent parameter should not be undefined (await parent must not throw)', async () => {
    ...
    const res = await fetch(`${baseUrl}/textjs-compat/metadata-parent-generate`)
  ...
  it('should pass searchParams to page generateMetadata()', async () => {
    const { html } = await fetchHtml(
      baseUrl,
      '/textjs-compat/metadata-generate-searchparams?q=hello',
    )
  ...
  it('generateMetadata searchParams defaults to empty when no query string', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-generate-searchparams')
  ...
  it('should render appLinks meta tags', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-applinks')
  ...
  it('should render twitter player card meta tags', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-twitter-player')
  ...
  it('should render twitter app card meta tags', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-twitter-app')
  ...
  //   Would need dedicated fixture pages — partially covered by existing text tests
  //
  ...
  //   Tests cache deduping with metadata — Playwright required
  //
  // N/A: Viewport tests
  //   Partially covered by existing text tests (tests/app-router.test.ts)
})
