 ...
 *
 * Fixture: __tests__/fixtures/app-basic/app/use-client-page-pathname/
...
describe('"use client" page component: usePathname() SSR (issue #688)', () => {
  ...
  it('adds CSP nonce to inline hydration and bootstrap scripts', async () => {
    ...
    expect(html).toContain(
      '<script nonce="text-test-nonce">((self[Symbol.for("text.navigationRuntime")]',
    )
    expect(html).toContain('.rsc.push(')
    expect(html).toContain('.done=true')
    expect(html).toMatch(/<link rel="modulepreload" nonce="text-test-nonce" href="[^"]+"/)
  ...
  // Ported from Text.js: test/production/app-dir/subresource-integrity/subresource-integrity.test.ts
