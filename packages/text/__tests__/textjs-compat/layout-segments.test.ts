/**
 * Text.js Compatibility Tests: useSelectedLayoutSegment(s) with route groups
 *
 ...
 *
 * Fixture structure:
 *   app/textjs-compat/hooks-segments/
...
describe('Text.js compat: useSelectedLayoutSegment(s)', () => {
  ...
  beforeAll(async () => {
    ...
    // Warm up
    await fetchHtml(baseUrl, '/textjs-compat/hooks-segments/first')
  }, 60_000)
  ...
  // Ported from Text.js: test/e2e/app-dir/hooks/hooks.test.ts
  ...
  describe('useSelectedLayoutSegments', () => {
    it('returns correct segments for simple path (outer layout)', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/hooks-segments/first')
    ...
    it('includes route group in segments (outer layout)', async () => {
      const { html } = await fetchHtml(
        baseUrl,
        '/textjs-compat/hooks-segments/first/slug2/second/a/b',
      )
    ...
    it('returns dynamic param segments (inner layout)', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/hooks-segments/first/slug1')
