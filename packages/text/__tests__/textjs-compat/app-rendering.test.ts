/**
 * Text.js Compatibility Tests: app-rendering
 *
 ...
 *
 * Fixture pages live in: fixtures/app-basic/app/textjs-compat/
 */
...
describe('Text.js compat: app-rendering', () => {
  ...
  // ── Root page ───────────────────────────────────────────────
  // Text.js: it('should serve app/page.server.js at /', ...)
  // We use a sub-route /textjs-compat since we share the app-basic fixture.
  it('should serve the textjs-compat root page', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat')
    expect(html).toContain('app/page.server.js')
  })

  // ── SSR only ────────────────────────────────────────────────
  describe('SSR only (revalidate = 0)', () => {
    // Text.js: it('should run data in layout and page', ...)
    ...
    it('should run async data in layout and page', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/ssr-only/nested')
    ...
    // Text.js: it('should run data fetch in parallel', ...)
    // Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-rendering/rendering.test.ts#L24-L33
    // NOTE: Delays reduced from 5s to 1s. The dev fixture can still report
    // about 2s of route compilation per request, so the threshold allows that
    // noise while still distinguishing parallel render (~3s) from sequential
    // render (~4s).
    it('should run data fetch in parallel (layout + page concurrent)', async () => {
      // Warm this specific route so the timing assertion measures render/data
      // concurrency rather than Vite's first-time module compilation.
      await fetchHtml(baseUrl, '/textjs-compat/ssr-only/slow')

      const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/ssr-only/slow')
      const timing = res.headers.get('x-text-timing')
      const renderMs = timing ? Number(timing.split(',')[2]) : Number.POSITIVE_INFINITY

      // Each part takes 1s. If sequential it would take 2s+.
      // If parallel it should complete in ~1s render time. Measure the
      // framework render phase instead of wall-clock request time so dev-only
      // Vite/RSC compilation noise cannot mask the concurrency signal.
  ...
  describe('static only (revalidate = false)', () => {
    ...
    it('should run async data in layout and page', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/static-only/nested')
      expect(html).toContain('hello from layout')
      expect(html).toContain('hello from page')
    })

    // Text.js: it('should run data in parallel ... during development', ...)
    ...
    it('should run data in parallel during development', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/static-only/slow')
      expect(html).toContain('hello from slow layout')
      expect(html).toContain('hello from slow page')
    })
  })
  ...
  describe('ISR (revalidate = 1)', () => {
    // Text.js: it('should revalidate the page when revalidate is configured', ...)
    ...
    it('should render ISR page with layout and page data', async () => {
      const { html } = await fetchHtml(baseUrl, '/textjs-compat/isr-multiple/nested')
      expect(html).toContain('hello from layout')
      expect(html).toContain('hello from page')
    ...
    // In dev mode, text always re-renders (no ISR caching), so timestamps should differ.
    ...
    //
    // ROOT CAUSE: text's RSC module instances persist across requests in dev mode.
    // Text.js re-executes server components fresh per request by invalidating the module cache.
    ...
    // (they may be related to the same module caching causing duplicate Rue instances).
    it.skip('should produce different timestamps on subsequent requests', async () => {
      const { html: html1 } = await fetchHtml(baseUrl, '/textjs-compat/isr-multiple/nested')
      ...
      const { html: html2 } = await fetchHtml(baseUrl, '/textjs-compat/isr-multiple/nested')
  ...
  // ── Mixed static and dynamic (skipped in Text.js too) ──────
  // Text.js: describe.skip('mixed static and dynamic', ...)
  ...
  describe.skip('mixed static and dynamic', () => {
    // SKIP: This is also skipped in the Text.js test suite.
