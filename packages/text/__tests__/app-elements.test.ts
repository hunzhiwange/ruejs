import { createElement, isRueRenderableHandle, type TextRenderable } from './rue-test-utils.js'
import fs from 'node:fs'
import path from 'node:path'
import { describe, expect, it } from 'vite-plus/test'
import { UNMATCHED_SLOT } from '../src/shims/slot.js'
import {
...
import { createAppSsrPayloadReader } from '../src/server/app-ssr-payload-reader-core.js'
import {
...
import { buildRenderObservation, createCacheEntryReuseProof } from '../src/server/cache-proof.js'
...
describe('AppElementsWire', () => {
  ...
  it('reads SSR payloads through injectable decode and thenable boundaries', () => {
    const stream = new ReadableStream<Uint8Array>()
    const payload = {
      [APP_INTERCEPTION_CONTEXT_KEY]: null,
      [APP_ROOT_LAYOUT_KEY]: '/',
      [APP_ROUTE_KEY]: AppElementsWire.encodeRouteId('/', null),
      'page:/': 'home-page',
    }
    // oxlint-disable-next-line unicorn/no-thenable -- This test verifies thenable payload handling.
    const thenable = { then: () => undefined } as PromiseLike<typeof payload>
    let decodeCalls = 0
    let readCalls = 0
    ...
    expect(readPayload()['page:/']).toBe('home-page')
    expect(readPayload()['page:/']).toBe('home-page')
  ...
  it('rejects malformed path URLs in explicit interception proof metadata', () => {
    ...
    for (const sourceMatchedUrl of malformedMatchedUrls) {
      ...
      ).toThrow('[text] Invalid __interception in App Router payload: expected path URLs')
    }
  })
  ...
  it.each([
    {
      ...
      message: '[text] Invalid __slotBindings in App Router payload: duplicate slot id',
    },
    {
...
describe('app elements payload helpers', () => {
  ...
  it('reads route metadata from the normalized payload', () => {
    const metadata = readAppElementsMetadata(
      normalizeAppElements({
        ...
        [APP_ROOT_LAYOUT_KEY]: '/(dashboard)',
        [APP_ROUTE_KEY]: 'route:/dashboard',
        'route:/dashboard': createElement('div', null, 'route'),
      }),
    )
  ...
  it('rejects payloads with a missing __route key', () => {
    ...
    ).toThrow('[text] Missing __route string in App Router payload')
  })
  ...
  it('rejects payloads with an invalid __rootLayout value', () => {
    ...
    ).toThrow('[text] Invalid __rootLayout in App Router payload: expected string or null')
  })
  ...
  it('rejects payloads with a missing __rootLayout key', () => {
    ...
    ).toThrow('[text] Missing __rootLayout key in App Router payload')
  })
  ...
  it('rejects payloads with an invalid __interceptionContext value', () => {
    ...
    ).toThrow('[text] Invalid __interceptionContext in App Router payload')
  })
  ...
  it('reads layoutIds from payload metadata', () => {
    const metadata = readAppElementsMetadata(
      normalizeAppElements({
        ...
        'route:/dashboard/settings': createElement('div', null, 'route'),
      }),
    )
  ...
  it('rejects invalid layoutIds metadata', () => {
    ...
    ).toThrow('[text] Invalid __layoutIds in App Router payload: expected layout id string[]')
  })
  ...
  it.each([
    {
      ...
      message: '[text] Invalid __slotBindings in App Router payload: expected array',
    },
    {
      label: 'non-object',
      value: ['slot:team:/dashboard'],
      message: '[text] Invalid __slotBindings in App Router payload: expected objects',
    },
    {
      label: 'non-slot id',
      value: [{ ownerLayoutId: 'layout:/dashboard', slotId: 'page:/dashboard', state: 'active' }],
      message: '[text] Invalid __slotBindings in App Router payload: expected slot ids',
    },
    {
      ...
      message: '[text] Invalid __slotBindings in App Router payload: expected owner layout ids',
    },
    {
      label: 'invalid state',
      value: [
      ...
      message: '[text] Invalid __slotBindings in App Router payload: expected state',
    },
    {
      label: 'duplicate slot id',
      value: [
  ...
  it.each([
    ...
    ).toThrow('[text] Invalid __layoutIds in App Router payload: expected layout ids')
  })
...
describe('isAppElementsRecord', () => {
  ...
  it('returns false for a Rue renderable handle', () => {
    const element = createElement('div', null, 'x')
...
describe('buildOutgoingAppPayload', () => {
  it('returns a non-record element unchanged (same identity)', () => {
    const element = createElement('div', null, 'page')
  ...
  it('does not mutate the input record', () => {
    const element: Record<string, TextRenderable> = {
      'layout:/': 'root-layout',
...
describe('artifact compatibility proof evaluation', () => {
  ...
  it('accepts rolling deploy payloads when compatibility is explicitly declared', () => {
    const current = createArtifactCompatibilityEnvelope({
      graphVersion: 'graph-text',
      deploymentVersion: 'deploy-canary',
      rootBoundaryId: 'root-text',
    ...
    expect(
      evaluateArtifactCompatibility(current, candidate, {
        compatibilityMap: {
          graphVersions: [['graph-current', 'graph-text']],
          deploymentVersions: [['deploy-stable', 'deploy-canary']],
          rootBoundaryIds: [['root-current', 'root-text']],
          renderEpochs: [['epoch-current', 'epoch-text']],
        },
      }),
