...
function createRenderer(overrides?: {
  ...
  globalNotFoundModule?: { default: TestServerComponent<any> } | null
  rootLayoutModules?: readonly ({ default: TestServerComponent<any> } | null | undefined)[]
  rootNotFoundModule?: { default: TestServerComponent<any> } | null
}) {
...
function NotFoundBoundary() {
  return createElement('p', { 'data-boundary': 'not-found' }, 'Missing page')
}

function RouteErrorBoundary({ error }: { error: Error }) {
  return createElement('p', { 'data-boundary': 'route-error' }, `route:${error.message}`)
}
...
function ParamsLayout({
  ...
  return createElement('div', { 'data-params-slug': params.slug ?? 'none' }, children)
}

function OverrideLayout({ children }: { children: TestServerNode }) {
  return createElement('div', { 'data-layout': 'override' }, children)
}

type TestModule = {
  default: TestServerComponent<any>
}
...
// Ported from Text.js: test/e2e/app-dir/default-error-page-ui/default-error-page-ui.test.ts
// https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/default-error-page-ui/default-error-page-ui.test.ts
//
// When the app does not define `error.tsx`, `global-error.tsx`, etc., text
// must still render the same default error UI Text.js ships, including the
...
describe('app fallback renderer default global error UI', () => {
  it('renders the built-in default global error UI for client errors', async () => {
    ...
    expect(html).toContain('--text-error-title')
...
// Regression for #1454 — default App Router 404 must match Text.js's built-in
...
describe('app fallback renderer default not-found UI', () => {
  it("renders the canonical 'This page could not be found.' body when no not-found.tsx exists", async () => {
    ...
    expect(response?.status).toBe(404)
    const html = await response?.text()
    // Canonical message must contain the trailing period to match Text.js
    // (see .textjs-ref/packages/text/src/client/components/builtin/not-found.tsx).
    expect(html).toContain('This page could not be found.')
    // Status code is surfaced as the <h1>.
    expect(html).toContain('404')
    // Old text default ("404 - Page not found") must NOT leak through.
...
// Mirrors Text.js 16 experimental.globalNotFound behavior.
// Ported from Text.js: test/e2e/app-dir/global-not-found/{basic,both-present,not-present}.
// Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/global-not-found
// In Text.js, `app/global-not-found.tsx` replaces the root layout for
// route-miss 404s — see createNotFoundLoaderTree in
// https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/app-render.tsx#L495-L520
describe('app fallback renderer with globalNotFoundModule', () => {
  function RootLayout({ children }: { children: TestServerNode }) {
  ...
  function GlobalNotFound() {
    return createElement(
      ...
      createElement(
        'body',
        null,
        createElement('h1', { id: 'global-error-title' }, 'global-not-found'),
      ),
    )
  }
  ...
  it('falls back to default not-found when global-not-found is absent and route is null', async () => {
    ...
    // user-defined root notFoundModule either, text renders its built-in
    // default not-found component (parity with Text.js's packaged
  ...
  it('does not use global-not-found for non-404 access fallbacks (403, 401)', async () => {
    ...
    function ForbiddenBoundary() {
      return createElement('p', { 'data-boundary': 'forbidden' }, 'Forbidden')
    }
