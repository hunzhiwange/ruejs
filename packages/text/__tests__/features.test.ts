...
import { renderAppServerElementToHtml } from './app-server-protocol-test-utils.js'
...
async function captureCompressedWebResponse(encoding: 'gzip' | 'br'): Promise<{
  ...
  const { sendWebResponse } = await import('../src/server/prod-server.js')
...
describe('parameterized redirects and rewrites', () => {
  ...
  beforeAll(async () => {
    ...
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-rr-'))

    // Symlink node_modules
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(tmpDir, 'node_modules'), 'junction')

    // text.config.mjs with parameterized redirects and rewrites
    await fsp.writeFile(
...
describe('external URL rewrites', () => {
  ...
  beforeAll(async () => {
    ...
    extTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-ext-rewrite-'))

    // Symlink node_modules
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(extTmpDir, 'node_modules'), 'junction')

    // text.config.mjs with external rewrites pointing to our local upstream
    await fsp.writeFile(
      path.join(extTmpDir, 'text.config.mjs'),
      `export default {
  async rewrites() {
    return [
...
describe('CSS Modules support (Pages Router)', () => {
  ...
  beforeAll(async () => {
    ...
    cssTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-css-'))
    ...
    // text.config.mjs
    await fsp.writeFile(path.join(cssTmpDir, 'text.config.mjs'), `export default {};`)
...
describe('ISR (Pages Router)', () => {
  ...
  it('serves stale content after TTL expires then regenerates', async () => {
    ...
    // Text request should be a HIT — background regen re-ran getStaticProps
    // and cached the fresh result.
    const res3 = await fetch(`${baseUrl}/isr-test`)
    expect(res3.status).toBe(200)
    expect(res3.headers.get('x-text-cache')).toBe('HIT')
  })
  ...
  it('background regeneration re-renders HTML with fresh props', async () => {
    ...
    // __TEXT_DATA__ must also contain the fresh timestamp, proving both the
    // server-rendered HTML and the hydration data are in sync.
    const textDataMatch = hitHtml.match(
      /window\.__TEXT_DATA__\s*=\s*(\{[\s\S]*?\})(?:;|<\/script>)/,
    )
    expect(textDataMatch).toBeTruthy()
    const textData = JSON.parse(textDataMatch![1])
    expect(textData.props.pageProps.timestamp).toBe(newTimestamp)
  })
...
describe('ISR (App Router)', () => {
  ...
  it('dev: does NOT write to or read from ISR cache (no X-Text-Cache header)', async () => {
    // In dev mode the production guard (statically compiled) prevents cache
    // reads and writes, so X-Text-Cache is absent on every request.
    const res1 = await fetch(`${baseUrl}/isr-test`)
  ...
  it('dev: RSC requests (.rsc suffix) return RSC stream with Cache-Control but no X-Text-Cache', async () => {
    const res = await fetch(`${baseUrl}/isr-test.rsc`, {
      headers: { Accept: 'text/x-component' },
    })
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/x-component')
    // ISR cache reads/writes are prod-only, so no X-Text-Cache in dev
    expect(res.headers.get('x-text-cache')).toBeNull()
  ...
  it('dev: RSC prefetch requests (Text-Router-Prefetch header) return RSC stream with Cache-Control', async () => {
    const res = await fetch(`${baseUrl}/isr-test.rsc`, {
      headers: { Accept: 'text/x-component', 'Text-Router-Prefetch': '1' },
    })
    expect(res.status).toBe(200)
    expect(res.headers.get('content-type')).toContain('text/x-component')
    // Prefetch RSC requests should also get Cache-Control (no X-Text-Cache in dev)
    expect(res.headers.get('cache-control')).toContain('s-maxage=1')
    expect(res.headers.get('x-text-cache')).toBeNull()
  })

  it('dev: pages without revalidate export emit no Cache-Control or X-Text-Cache headers', async () => {
...
describe('i18n config parsing', () => {
  it('parses i18n config from text.config.js', async () => {
...
describe('extractLocaleFromUrl', () => {
  let extractLocaleFromUrl: typeof import('../src/server/dev-server.js').extractLocaleFromUrl

  beforeAll(async () => {
    const mod = await import('../src/server/dev-server.js')
...
describe('detectLocaleFromHeaders', () => {
  let detectLocaleFromHeaders: typeof import('../src/server/dev-server.js').detectLocaleFromHeaders
...
describe('i18n routing (Pages Router)', () => {
  ...
  beforeAll(async () => {
    ...
    i18nTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-i18n-'))

    // Symlink node_modules from root
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(i18nTmpDir, 'node_modules'), 'junction')

    // text.config.mjs with i18n
    await fsp.writeFile(
      path.join(i18nTmpDir, 'text.config.mjs'),
      `export default {
  i18n: {
    locales: ["en", "fr", "de"],
    defaultLocale: "en",
...
export default function About({ locale, locales, defaultLocale }) {
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.race([i18nServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
  ...
  it('redirects to detected locale based on Accept-Language header', async () => {
    ...
    // Text.js only auto-detects locale on the application root.
    expect(res.status).toBe(200)
...
describe('i18n domain routing (Pages Router)', () => {
  ...
  beforeAll(async () => {
    ;({
      ...
      tmpDir: domainTmpDir,
      port: domainPort,
    } = await startDomainFixtureServer(PAGES_I18N_DOMAINS_FIXTURE_DIR, 'text-i18n-domain-'))
  }, 30000)

  afterAll(async () => {
    try {
      ;(domainServer?.httpServer as any)?.closeAllConnections?.()
      await Promise.race([domainServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
  ...
  it('uses the matched domain default locale in request context and __TEXT_DATA__', async () => {
...
describe('Link locale prop', () => {
  ...
  beforeAll(async () => {
    ...
    linkLocaleTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-link-locale-'))

    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(linkLocaleTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(
      path.join(linkLocaleTmpDir, 'text.config.mjs'),
      `export default {
  i18n: {
    locales: ["en", "fr", "de"],
    defaultLocale: "en",
...
describe('i18n localeDetection: false', () => {
  ...
  beforeAll(async () => {
    ...
    noDetectTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-i18n-nodetect-'))

    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(noDetectTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(
      path.join(noDetectTmpDir, 'text.config.mjs'),
      `export default {
  i18n: {
    locales: ["en", "fr"],
    defaultLocale: "en",
...
describe('basePath support (Pages Router)', () => {
  it('resolveTextConfig correctly resolves basePath', async () => {
    const { resolveTextConfig } = await import('../src/config/text-config.js')

    // Default: empty basePath
    const defaultConfig = await resolveTextConfig({})
    expect(defaultConfig.basePath).toBe('')

    // With basePath configured
    const withBasePath = await resolveTextConfig({ basePath: '/app' })
    expect(withBasePath.basePath).toBe('/app')

    // basePath must start with / (Text.js requirement)
    const withSlash = await resolveTextConfig({ basePath: '/docs' })
    expect(withSlash.basePath).toBe('/docs')
  })

  it('resolveTextConfig correctly resolves trailingSlash', async () => {
    const { resolveTextConfig } = await import('../src/config/text-config.js')
    ...
    const withTrailing = await resolveTextConfig({ trailingSlash: true })
...
describe('build-time defines (Pages Router)', () => {
  ...
  it('basePath define is injected into client code', async () => {
    ...
    const defineKey = 'process.env.__TEXT_ROUTER_BASEPATH'
    expect(server.config.define?.[defineKey]).toBe(JSON.stringify(''))
  })

  it('App Shells define is always false', async () => {
    // Plumbing-only upstream; text always reports false so client gating
    // code never trips. See issue #1405.
    const defineKey = 'process.env.__TEXT_APP_SHELLS'
...
describe('basePath HTTP routing (Pages Router)', () => {
  ...
  beforeAll(async () => {
    ...
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-bp-'))

    // Symlink node_modules from project root so Rue etc. are resolvable
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(tmpDir, 'node_modules'), 'junction')

    // text.config.mjs with basePath
    await fsp.writeFile(
      path.join(tmpDir, 'text.config.mjs'),
...
`,
  ...
  it('process.env.__TEXT_ROUTER_BASEPATH define is /app', () => {
    const define = bpServer.config.define?.['process.env.__TEXT_ROUTER_BASEPATH']
...
describe('basePath with nested path (/docs/v2)', () => {
  ...
  beforeAll(async () => {
    ...
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-bp-nested-'))
  ...
  afterAll(async () => {
    ...
    try {
      ...
      await Promise.race([nestedServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore close errors */
    }
    const fsp = await import('node:fs/promises')
...
describe('basePath + trailingSlash interaction', () => {
  ...
  beforeAll(async () => {
    ...
    tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-bp-ts-'))
...
describe('metadata title templates', () => {
  let mergeMetadata: typeof import('../src/shims/metadata.js').mergeMetadata
  let mergeMetadataEntries: typeof import('../src/shims/metadata.js').mergeMetadataEntries

  beforeAll(async () => {
    const mod = await import('../src/shims/metadata.js')
  ...
  it('applies ancestor title template to child layout default title', () => {
    // Text.js resolveTitle() applies the stashed ancestor template to title.default:
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/metadata/resolvers/resolve-title.ts
  ...
  it('later entries override earlier for non-title fields', () => {
    ...
    // Text.js postProcessMetadata also fills openGraph.description from
...
// Ported from Text.js behavior:
// - docs: https://textjs.org/docs/app/api-reference/functions/generate-metadata#merging
// - source: packages/text/src/lib/metadata/resolve-metadata.ts
// https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/metadata/resolve-metadata.ts
...
// Ported from Text.js behavior: packages/text/src/lib/metadata/resolve-metadata.ts
// https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/metadata/resolve-metadata.ts

describe('metadata OG/Twitter inheritance', () => {
  let mergeMetadata: typeof import('../src/shims/metadata.js').mergeMetadata
  let renderMetadataToHtml: typeof import('../src/shims/metadata.js').renderMetadataToHtml
...
describe('MetadataHead rendering', () => {
  ...
  it('renders generator meta tag', () => {
    const html = renderMetadataToHtml({ generator: 'Text.js' })
    expect(html).toContain('name="generator"')
    expect(html).toContain('content="Text.js"')
  })

  it('renders application-name meta tag', () => {
    const html = renderMetadataToHtml({ applicationName: 'My App' })
  ...
  it('renders format-detection meta tag', () => {
    const html = renderMetadataToHtml({ formatDetection: { telephone: false, email: false } })
  ...
  it('renders top-level icon shorthand metadata', () => {
    const html = renderMetadataToHtml({ icons: '/manual-icon.png' })
  ...
  it('renders alternate hreflang links', () => {
    ...
    // Rue renders hrefLang in camelCase (hrefLang or hreflang depending on version)
  ...
  it('renders twitter:site and twitter:creator:id', () => {
    ...
    expect(html).toContain('content="@textjs"')
  ...
  it('accepts URL objects for canonical and openGraph.url', () => {
    // Text.js allows string | URL for URL fields; passing a URL object must not throw
    const html = renderMetadataToHtml({
  ...
  it('renders manifest link', () => {
    const html = renderMetadataToHtml({ manifest: '/manifest.json' })
    expect(html).toContain('rel="manifest"')
    expect(html).toContain('href="/manifest.json"')
  })

  it('renders other meta with array values', () => {
    const html = renderMetadataToHtml({ other: { custom: ['val1', 'val2'] } })
...
describe('ViewportHead rendering', () => {
  let ViewportHead: typeof import('../src/shims/metadata.js').ViewportHead
  let mergeViewport: typeof import('../src/shims/metadata.js').mergeViewport
...
describe('fetch cache (extended fetch with text options)', () => {
  ...
  it('exports withFetchCache, runWithFetchCache, getOriginalFetch, getCollectedFetchTags', async () => {
    const mod = await import('../src/shims/fetch-cache.js')
  ...
  it('passes through fetch without text options to persistent cache but dedupes within a render', async () => {
    const { withFetchCache } = await import('../src/shims/fetch-cache.js')
  ...
  it('caches fetch with { text: { revalidate } }', async () => {
    const { withFetchCache } = await import('../src/shims/fetch-cache.js')
  ...
  it('text.revalidate: false bypasses persistent cache but dedupes within a render', async () => {
    const { withFetchCache } = await import('../src/shims/fetch-cache.js')
    const { setCacheHandler, MemoryCacheHandler } = await import('../src/shims/cache.js')
    ...
    let cleanup = withFetchCache()
    try {
      await fetch(`${mockServerUrl}/no-rev`, { text: { revalidate: false } })
      await fetch(`${mockServerUrl}/no-rev`, { text: { revalidate: false } })
      expect(fetchCallCount).toBe(1)
  ...
  it('text.revalidate: 0 bypasses persistent cache but dedupes within a render', async () => {
    const { withFetchCache } = await import('../src/shims/fetch-cache.js')
    const { setCacheHandler, MemoryCacheHandler } = await import('../src/shims/cache.js')
    ...
    let cleanup = withFetchCache()
    try {
      await fetch(`${mockServerUrl}/zero`, { text: { revalidate: 0 } })
      await fetch(`${mockServerUrl}/zero`, { text: { revalidate: 0 } })
      expect(fetchCallCount).toBe(1)
  ...
  it('revalidateTag invalidates fetch cache entries', async () => {
    ...
    try {
      ...
      const resp1 = await fetch(`${mockServerUrl}/tagged`, {
        text: { revalidate: 3600, tags: ['posts'] },
      })
      const data1 = await resp1.json()
      expect(data1.count).toBe(1)
      expect(fetchCallCount).toBe(1)
  ...
  it('collects fetch tags during render pass', async () => {
    const { withFetchCache, getCollectedFetchTags } = await import('../src/shims/fetch-cache.js')
    const { setCacheHandler, MemoryCacheHandler } = await import('../src/shims/cache.js')
    ...
    const cleanup = withFetchCache()
    try {
      await fetch(`${mockServerUrl}/a`, { text: { revalidate: 60, tags: ['tag-a', 'tag-b'] } })
      await fetch(`${mockServerUrl}/b`, { text: { revalidate: 60, tags: ['tag-b', 'tag-c'] } })
  ...
  it('different URLs produce separate cache entries', async () => {
    ...
    const cleanup = withFetchCache()
    try {
      await fetch(`${mockServerUrl}/url-a`, { text: { revalidate: 60 } })
      await fetch(`${mockServerUrl}/url-b`, { text: { revalidate: 60 } })
  ...
  it('fetch with only tags (no revalidate) caches indefinitely', async () => {
    ...
    const cleanup = withFetchCache()
    try {
      await fetch(`${mockServerUrl}/tags-only`, { text: { tags: ['items'] } })
      await fetch(`${mockServerUrl}/tags-only`, { text: { tags: ['items'] } })
  ...
  it('cleanup clears per-request tag state', async () => {
    ...
    setCacheHandler(new MemoryCacheHandler())
    const cleanup = withFetchCache()
    await fetch(`${mockServerUrl}/cleanup-tag-test`, { text: { tags: ['cleanup-t'] } })
  ...
  it('runWithFetchCache isolates tags and returns result', async () => {
    const { runWithFetchCache, getCollectedFetchTags } = await import('../src/shims/fetch-cache.js')
    const { setCacheHandler, MemoryCacheHandler } = await import('../src/shims/cache.js')
    ...
    const result = await runWithFetchCache(async () => {
      const resp = await fetch(`${mockServerUrl}/auto`, { text: { revalidate: 60 } })
      return resp.json()
    })
  ...
  it('strips text property before passing to real fetch', async () => {
    const { withFetchCache } = await import('../src/shims/fetch-cache.js')
    const { setCacheHandler, MemoryCacheHandler } = await import('../src/shims/cache.js')

    setCacheHandler(new MemoryCacheHandler())

    // This test verifies the real fetch doesn't receive the `text` property
    // which would cause warnings in some environments. If it throws, the test fails.
    const cleanup = withFetchCache()
    try {
      const resp = await fetch(`${mockServerUrl}/strip`, {
        text: { revalidate: 60, tags: ['test'] },
        method: 'GET',
      })
...
describe('instrumentation.ts support', () => {
  it('exports findInstrumentationFile', async () => {
    const mod = await import('../src/server/instrumentation.js')
    expect(typeof mod.findInstrumentationFile).toBe('function')
  })

  it('findInstrumentationFile returns null when no file exists', async () => {
    const { findInstrumentationFile } = await import('../src/server/instrumentation.js')
  ...
  it('findInstrumentationFile detects instrumentation.ts', async () => {
    ...
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-inst-'))
    fs.writeFileSync(
  ...
  it('runInstrumentation calls register() and stores onRequestError', async () => {
    const { runInstrumentation, getOnRequestErrorHandler } =
      await import('../src/server/instrumentation.js')
  ...
  it('runInstrumentation handles missing register gracefully', async () => {
    const { runInstrumentation } = await import('../src/server/instrumentation.js')
  ...
  it('runInstrumentation handles import errors gracefully', async () => {
    ...
    try {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        '[text] Failed to load instrumentation:',
...
describe('production server compression', () => {
  it('negotiateEncoding returns br when Accept-Encoding includes br', async () => {
    const { negotiateEncoding } = await import('../src/server/prod-server.js')
  ...
  it('COMPRESSIBLE_TYPES includes expected content types', async () => {
    const { COMPRESSIBLE_TYPES } = await import('../src/server/prod-server.js')
  ...
  it('COMPRESS_THRESHOLD is a reasonable minimum', async () => {
    const { COMPRESS_THRESHOLD } = await import('../src/server/prod-server.js')
  ...
  it('sendCompressed compresses text/html with gzip', async () => {
    const { sendCompressed } = await import('../src/server/prod-server.js')
...
describe('Set-Cookie header preservation in prod-server', () => {
  it('mergeResponseHeaders preserves multiple Set-Cookie from response', async () => {
    const { mergeResponseHeaders } = await import('../src/server/prod-server.js')
  ...
  it('mergeWebResponse preserves the original body stream while applying header overrides', async () => {
    const { mergeWebResponse } = await import('../src/server/prod-server.js')
  ...
  it('mergeWebResponse cancels discarded body streams for no-body statuses', async () => {
    ...
    const merged = mergeWebResponse({}, response, 204)
  ...
  it('mergeWebResponse strips stale content-length only for tagged streamed Pages HTML', async () => {
    ...
    ) as Response & { __textStreamedHtmlResponse?: boolean }
    response.__textStreamedHtmlResponse = true
...
describe('host header poisoning prevention', () => {
  it('resolveHost ignores X-Forwarded-Host by default', async () => {
    const { resolveHost } = await import('../src/server/prod-server.js')
    const req = {
      headers: {
  ...
  it('resolveHost trusts X-Forwarded-Host when it is in the trusted hosts set', async () => {
    const { resolveHost, trustedHosts } = await import('../src/server/prod-server.js')
...
describe('X-Forwarded-Proto trust proxy gating', () => {
  ...
  it('trustProxy is true when TEXT_TRUSTED_HOSTS is non-empty', async () => {
    vi.stubEnv('TEXT_TRUSTED_HOSTS', 'example.com')
    vi.resetModules()
    const mod = await import('../src/server/prod-server.js')
  ...
  it('trustProxy is false when TEXT_TRUSTED_HOSTS is empty string', async () => {
    vi.stubEnv('TEXT_TRUSTED_HOSTS', '')
    vi.resetModules()
    const mod = await import('../src/server/prod-server.js')
    expect(mod.trustProxy).toBe(false)
...
// ---------------------------------------------------------------------------
// Edge cases from Text.js test suite
// ---------------------------------------------------------------------------

describe('Text.js edge cases', () => {
  ...
  it('getStaticProps query params do not leak into params', async () => {
    ...
    // Check the __TEXT_DATA__ doesn't leak query params into getStaticProps
    const dataMatch = html.match(/__TEXT_DATA__\s*=\s*(\{[^<]+\})/)
    expect(dataMatch).not.toBeNull()
    const data = JSON.parse(dataMatch![1])
...
describe('multi-byte character SSR', () => {
  ...
  beforeAll(async () => {
    ...
    mbTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mb-'))
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(mbTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(path.join(mbTmpDir, 'text.config.mjs'), `export default {};`)
...
describe('i18n cross-locale redirect from getServerSideProps', () => {
  ...
  beforeAll(async () => {
    ...
    localeRedirectTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-locale-redirect-'))
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(localeRedirectTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(
      path.join(localeRedirectTmpDir, 'text.config.mjs'),
      `export default {
  i18n: {
    locales: ["en", "fr", "de"],
    defaultLocale: "en",
...
describe('applyNavigationLocale', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as any).__TEXT_DEFAULT_LOCALE__ = 'en'
    ;(globalThis as any).__TEXT_DATA__ = undefined
  })

  afterEach(() => {
    delete (globalThis as any).__TEXT_DEFAULT_LOCALE__
  ...
  it('returns an absolute cross-domain URL when the locale belongs to another domain', () => {
    ;(globalThis as any).__TEXT_DATA__ = {
      domainLocales: [
...
// ---------------------------------------------------------------------------
// parseCookieLocale (TEXT_LOCALE cookie support)
// ---------------------------------------------------------------------------
...
describe('parseCookieLocale', () => {
  ...
  it('returns null when cookie header has no TEXT_LOCALE', () => {
    expect(parseCookieLocale({ headers: { cookie: 'foo=bar; baz=qux' } }, config)).toBeNull()
  })

  it('returns locale from TEXT_LOCALE cookie', () => {
    expect(parseCookieLocale({ headers: { cookie: 'TEXT_LOCALE=fr' } }, config)).toBe('fr')
  })

  it('returns locale when TEXT_LOCALE is among multiple cookies', () => {
    expect(
      parseCookieLocale({ headers: { cookie: 'theme=dark; TEXT_LOCALE=de; session=abc' } }, config),
    ).toBe('de')
  })

  it('returns null for invalid locale in cookie', () => {
    expect(parseCookieLocale({ headers: { cookie: 'TEXT_LOCALE=es' } }, config)).toBeNull()
  })
  ...
  it('returns default locale when cookie matches default', () => {
    expect(parseCookieLocale({ headers: { cookie: 'TEXT_LOCALE=en' } }, config)).toBe('en')
  })
})

// ---------------------------------------------------------------------------
// TEXT_LOCALE cookie integration (i18n redirect behavior)
// ---------------------------------------------------------------------------

describe('TEXT_LOCALE cookie redirect behavior', () => {
  ...
  beforeAll(async () => {
    ...
    cookieTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-cookie-locale-'))

    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(cookieTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(
      path.join(cookieTmpDir, 'text.config.mjs'),
      `export default {
  i18n: {
    locales: ["en", "fr", "de"],
    defaultLocale: "en",
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.race([cookieServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
  ...
  it('TEXT_LOCALE cookie redirects to non-default locale', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: { Cookie: 'TEXT_LOCALE=fr' },
    })
    expect(res.status).toBe(307)
    expect(res.headers.get('location')).toBe('/fr')
  })

  it('TEXT_LOCALE cookie redirects on non-root paths', async () => {
    const res = await fetch(`${cookieBaseUrl}/about`, {
      redirect: 'manual',
      headers: { Cookie: 'TEXT_LOCALE=de' },
    })
    expect(res.status).toBe(200)
    expect(res.headers.get('location')).toBeNull()
  })

  it('TEXT_LOCALE cookie matching default locale does NOT redirect', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: { Cookie: 'TEXT_LOCALE=en' },
    })
    expect(res.status).toBe(200)
  })

  it('TEXT_LOCALE cookie takes priority over Accept-Language', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: {
        Cookie: 'TEXT_LOCALE=de',
        'Accept-Language': 'fr',
      },
    })
    expect(res.status).toBe(307)
  ...
  it('TEXT_LOCALE cookie with default locale suppresses Accept-Language redirect', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: {
        Cookie: 'TEXT_LOCALE=en',
  ...
  it('invalid TEXT_LOCALE cookie falls through to Accept-Language', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: {
        Cookie: 'TEXT_LOCALE=es',
        'Accept-Language': 'fr',
      },
    })
  ...
  it('no TEXT_LOCALE cookie falls through to Accept-Language', async () => {
    const res = await fetch(`${cookieBaseUrl}/`, {
      redirect: 'manual',
      headers: {
        Cookie: 'theme=dark',
  ...
  it('TEXT_LOCALE does not redirect when URL already has locale prefix', async () => {
...
describe('chained middleware → config rewrites', () => {
  ...
  beforeAll(async () => {
    ...
    chainTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-chain-rewrite-'))
    ...
    await fsp.writeFile(
      path.join(chainTmpDir, 'text.config.mjs'),
      `export default {
  async rewrites() {
    return [
...
export function middleware(request) {
  ...
  if (url.pathname === "/original") {
    return TextResponse.rewrite(new URL("/rewrite-source", request.url));
  }
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.race([chainServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
...
describe('middleware rewriteStatus propagation (Pages Router dev)', () => {
  ...
  beforeAll(async () => {
    ...
    statusTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-rewrite-status-'))
...
export function middleware(request) {
  ...
  if (url.pathname === "/blocked") {
    return TextResponse.rewrite(new URL("/allowed", request.url), { status: 403 });
  }
  if (url.pathname === "/text-status") {
    return TextResponse.text({ status: 404 });
  }
  return TextResponse.text();
}`,
    )
    ...
    await fsp.writeFile(
      path.join(statusTmpDir, 'pages', 'text-status.tsx'),
      `export default function TextStatus() { return <h1>TEXT STATUS PAGE</h1>; }`,
    )
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.race([statusServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
...
// ---------------------------------------------------------------------------
// Additional Text.js edge cases (batch 2)
// ---------------------------------------------------------------------------
...
describe('Pages Router edge cases (batch 2)', () => {
  ...
  beforeAll(async () => {
    ...
    edgeTmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-edge2-'))

    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(edgeTmpDir, 'node_modules'), 'junction')

    await fsp.writeFile(path.join(edgeTmpDir, 'text.config.mjs'), `export default {};`)
...
export default function NestedProps({ user }) {
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.race([edgeServer?.close(), new Promise(resolve => setTimeout(resolve, 5000))])
    } catch {
      /* ignore */
    }
    const fsp = await import('node:fs/promises')
...
describe('.env file loading (Issue #228)', () => {
  ...
  beforeAll(async () => {
    ...
    tmpDir = await fsp.mkdtemp(path.join(tmpRoot, 'text-env-'))

    // Symlink node_modules
    const rootNodeModules = path.resolve(import.meta.dirname, '../../../node_modules')
    await fsp.symlink(rootNodeModules, path.join(tmpDir, 'node_modules'), 'junction')

    // .env file with TEXT_PUBLIC_ and server-only vars
    await fsp.writeFile(
      path.join(tmpDir, '.env'),
      `TEXT_PUBLIC_APP_NAME=text-test-app
...
`,
  ...
  return {
    props: {
      publicVar: process.env.TEXT_PUBLIC_APP_NAME ?? "NOT_LOADED",
...
`,
  ...
  afterAll(async () => {
    ...
    delete process.env.TEXT_PUBLIC_APP_NAME
  ...
  it('TEXT_PUBLIC_ vars from .env are available via process.env', () => {
    // Check the Vite define config — TEXT_PUBLIC_ vars from .env should
    // be registered as define entries for client-side inlining.
    const defineKey = 'process.env.TEXT_PUBLIC_APP_NAME'
    const defineValue = envServer.config.define?.[defineKey]
    expect(defineValue).toBe(JSON.stringify('text-test-app'))
  })

  it('server-only vars from .env are NOT exposed in client defines', () => {
    // Only TEXT_PUBLIC_* vars should be inlined into the client bundle.
