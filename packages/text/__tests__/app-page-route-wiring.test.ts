...
function containsElementType(node: unknown, type: unknown): boolean {
  ...
  return (
    element.type === type ||
    containsElementType(element.props.children, type) ||
    containsElementType(element.props.fallback, type)
  )
}
...
type InspectableElement<P extends Record<string, unknown> = InspectableElementProps> = {
  key?: string | null
  type: unknown
  props: P
}
...
function findElement(
  node: unknown,
  predicate: (element: InspectableElement<InspectableElementProps>) => boolean,
): InspectableElement<InspectableElementProps> | null {
  if (Array.isArray(node)) {
...
function findElementByTypeName(
  node: unknown,
  typeName: string,
): InspectableElement<Record<string, unknown>> | null {
...
function findSuspenseWithFallback(
  ...
  const match = findElement(node, element => {
    if (element.type !== AppServerSuspense) {
      return false
    }
...
function renderHtml(node: TestServerNode): Promise<string> {
  return renderAppServerElementToHtmlAsync(node)
}
...
describe('app page route wiring helpers', () => {
  it('renders generated metadata in a hidden body outlet for streaming-capable requests', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-streaming/metadata-streaming.test.ts
    ...
    expect(html).toMatch(/<div hidden(?:="")?><title>generated page<\/title><\/div>/)
  })

  it('renders generated metadata in the head for configured html-limited bots', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-streaming/metadata-streaming-customized-rule.test.ts
  ...
  it('falls back to the default html-limited bot list for an empty config string', async () => {
    // Text.js normalizes a falsy htmlLimitedBots config to the default bot regex.
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/lib/streaming-metadata.ts
    const html = await buildGeneratedMetadataRouteHtml('HeadlessChrome', '')
  ...
  it('omits slot key on RSC request when slot has only default.tsx (no page) and slot is already mounted', () => {
    ...
    // is already mounted (browser told us via X-Text-Mounted-Slots header).
  ...
  it('keys template slots with the semantic segment state key', () => {
    function LeafTemplate(props: { children?: TestServerNode }) {
  ...
  it('does not collide route reset keys across branches with the same dynamic leaf value', () => {
    ...
    expect(postsLoadingBoundary?.key).toBe(JSON.stringify(['reset-collision', 'posts', 'id|123|d']))
  ...
  it('does not collide route reset keys across static branches with the same leaf segment', () => {
    ...
    expect(adminLoadingBoundary?.key).toBe(JSON.stringify(['reset-collision', 'admin', 'settings']))
