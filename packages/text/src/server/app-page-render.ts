...
type RenderAppPageLifecycleOptions = {
  ...
  renderToReadableStream: (
    element: TextRenderable | AppOutgoingElements,
...
function textToReadableStream(value: string): ReadableStream<Uint8Array> {
...
function isDevTextCompatRuntimeShellError(error: unknown): boolean {
...
function createAppPageArtifactCompatibility(
  ...
  return createArtifactCompatibilityEnvelope({
    ...
    deploymentVersion: process.env.__TEXT_BUILD_ID ?? null,
    rootBoundaryId,
  })
}
 ...
 *
 * Dedups against serialized payload error chunks: if the recorded error
 * already carries a `digest`, the server components error handler has already
 * stamped it and emitted it into the RSC stream. Skipping `console.error`
 * prevents double-logging. Caught cases (no digest) still surface here.
 *
 * Ported from Text.js:
...
function wrapRscResponseForDevErrorReporting(
  ...
  const onConsumed = () => {
    ...
    if (!error) return
    // Dedup: the payload renderer already emitted this error chunk.
    if (!hasDigest(error)) {
      console.error('[text] Invalid dynamic usage:', error)
    }
  }
...
export async function renderAppPageLifecycle(
  ...
  if (options.isRscRequest) {
    ...
    // errors after the stream is consumed. This mirrors Text.js behavior where
    // workStore.invalidDynamicUsageError is checked after the accumulated chunks
    // promise resolves (the dynamic RSC result path in app-render.tsx).
    // Ported from Text.js: https://github.com/vercel/next.js/commit/f5e54c06726b571a042fce67417e40a29f6b8689
    //
    ...
    // if uncaught by user code. Full parity with Text.js would require checking
  ...
  let capturedSsrError: unknown = null
  ...
  const htmlRender = await renderAppPageHtmlStreamWithRecovery({
    ...
    async renderHtmlStream() {
      ...
      return renderAppPageHtmlStream({
        ...
        ssrPayload: isAppElementsRecord(outgoingElement)
          ? (outgoingElement as AppWireElements)
          : undefined,
        ssrPayloadDecoder: options.ssrPayloadDecoder,
        onSsrError(error) {
          if (capturedSsrError === null) {
            capturedSsrError = error
          }
        },
        ssrHandler,
        waitForAllReady:
          options.isPrerender ||
          (options.isProduction && (!options.hasLoadingBoundary || !options.routeHasLocalBoundary)),
      })
    },
  ...
  if (!htmlStream) {
    throw new Error('[text] Expected an HTML stream when no fallback response was returned')
  }

  if ((options.isProduction || options.isPrerender) && capturedSsrError !== null) {
    const specialError = resolveAppPageSpecialError(capturedSsrError)
    void htmlStream.cancel().catch(() => {})
    if (specialError) {
      return options.renderPageSpecialError(specialError)
    }

    const cleanResponse = await options.renderErrorBoundaryResponse(capturedSsrError)
    if (cleanResponse) {
  ...
  // Text.js's `app-render.tsx:4293` catch shape: by the time the SSR shell
  ...
  // the SSR onError hook and been captured by the tracker. Convert that to a
  ...
  if (
    shouldRerenderAppPageWithGlobalError({
      capturedError: capturedRscError,
      hasLocalBoundary: options.routeHasLocalBoundary,
    }) &&
    (options.isProduction ||
      options.isPrerender ||
      !isDevTextCompatRuntimeShellError(capturedRscError))
  ) {
    const cleanResponse = await options.renderErrorBoundaryResponse(capturedRscError)
    if (cleanResponse) {
      return cleanResponse
    }
  }
  ...
  if (options.isPrerender === true) {
    ...
    htmlStream = textToReadableStream(html)
  }
