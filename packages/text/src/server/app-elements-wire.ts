...
export const APP_UNMATCHED_SLOT_WIRE_VALUE = '__TEXT_UNMATCHED_SLOT__'

export const UNMATCHED_SLOT = Symbol.for('text.unmatchedSlot')
...
export function normalizeAppElementsSlotBindings(
  ...
  for (const binding of slotBindings) {
    if (seenSlotIds.has(binding.slotId)) {
      throw new Error('[text] Invalid __slotBindings in App Router payload: duplicate slot id')
    }
    ...
    if (
      ...
      throw new Error(
        '[text] Invalid __slotBindings in App Router payload: owner layout id missing from __layoutIds',
      )
    }
...
/**
 * Per-layout static/dynamic flags. `"s"` = static (skippable on text nav);
 ...
 *
 *   3. WIRE    — the active payload renderer serializes the record as row 0.
 *
...
/**
 * The outgoing wire payload shape. Includes renderable values for the
...
type AppElementsWireCodec = {
  ...
  encodeOutgoingPayload(input: {
    element:
      ...
      | Readonly<
          Record<
            string,
            TextRenderable | AppElementsInterception | readonly AppElementsSlotBinding[]
          >
        >
    artifactCompatibility?: ArtifactCompatibilityEnvelope
    cacheEntryReuseProof?: CacheEntryReuseProof
  ...
  }): TextRenderable | AppOutgoingElements
...
export function normalizeAppElements(elements: AppWireElements): AppElements {
  ...
  for (const [key, value] of Object.entries(elements)) {
    if (
      isTransportTextValue(value) ||
      containsTransportTextValue(value) ||
  ...
  for (const [key, value] of Object.entries(elements)) {
    const decoded = decodeTransportTextValue(value)
    normalized[key] =
      isAppElementsWireSlotId(key) && decoded === APP_UNMATCHED_SLOT_WIRE_VALUE
        ? UNMATCHED_SLOT
        : (decoded as AppElementValue)
  }
...
function containsTransportTextValue(value: unknown): boolean {
  if (isTransportTextValue(value)) return true
  if (Array.isArray(value)) return value.some(item => containsTransportTextValue(item))
  if (typeof value !== 'object' || value === null) return false
  for (const item of Object.values(value)) {
    if (containsTransportTextValue(item)) return true
  }
  return false
}

function decodeTransportTextValue(value: unknown): unknown {
  if (isTransportTextValue(value)) {
    return decodeTransportTextValue(value.value)
  }
  if (Array.isArray(value)) {
    return value.map(item => decodeTransportTextValue(item))
  }
  ...
  for (const [key, item] of Object.entries(value)) {
    decoded[key] = decodeTransportTextValue(item)
  }
  return decoded
}
...
function parseLayoutIds(value: unknown): readonly string[] {
  ...
  if (!Array.isArray(value)) {
    throw new Error('[text] Invalid __layoutIds in App Router payload: expected layout id string[]')
  }
  ...
  for (const entry of value) {
    if (typeof entry !== 'string') {
      throw new Error(
        '[text] Invalid __layoutIds in App Router payload: expected layout id string[]',
      )
    }

    const parsed = parseAppElementsWireElementKey(entry)
    if (parsed?.kind !== 'layout') {
      throw new Error('[text] Invalid __layoutIds in App Router payload: expected layout ids')
    }
...
function parseSlotBindings(
  ...
  if (value === undefined) return []
  if (!Array.isArray(value)) {
    throw new Error('[text] Invalid __slotBindings in App Router payload: expected array')
  }

  const slotBindings: AppElementsSlotBinding[] = []
  for (const entry of value) {
    if (!isRecord(entry)) {
      throw new Error('[text] Invalid __slotBindings in App Router payload: expected objects')
    }

    const slotId = entry.slotId
    if (typeof slotId !== 'string' || parseAppElementsWireElementKey(slotId)?.kind !== 'slot') {
      throw new Error('[text] Invalid __slotBindings in App Router payload: expected slot ids')
    }
    ...
    if (
      ...
      throw new Error(
        '[text] Invalid __slotBindings in App Router payload: expected owner layout ids',
      )
    }

    const state = entry.state
    if (!isSlotBindingState(state)) {
      throw new Error('[text] Invalid __slotBindings in App Router payload: expected state')
    }
...
function readRequiredInterceptionString(
  ...
  if (typeof value !== 'string') {
    throw new Error('[text] Invalid __interception in App Router payload: expected strings')
  }
  return value
}
...
function parseInterceptionMatchedUrl(value: string): string {
  if (!isInterceptionMatchedUrlPath(value)) {
    throw new Error('[text] Invalid __interception in App Router payload: expected path URLs')
  }
  return value
}
...
function parseInterceptionRouteId(value: string, matchedUrl: string): string {
  ...
  if (
    ...
    throw new Error('[text] Invalid __interception in App Router payload: expected route ids')
  }
  return value
}
...
function parseInterceptionSlotId(value: string): string {
  if (parseAppElementsWireElementKey(value)?.kind !== 'slot') {
    throw new Error('[text] Invalid __interception in App Router payload: expected slot id')
  }
  return value
}
...
function parseInterceptionMetadata(value: unknown): AppElementsInterception | null {
  ...
  if (!isRecord(value)) {
    throw new Error('[text] Invalid __interception in App Router payload: expected object')
  }
 ...
 * Used to distinguish the App Router payload object from bare framework elements at
 * the render boundary. Narrows to `Readonly<Record<string, unknown>>` because
 * the outgoing payload carries heterogeneous values (renderables for the rendered
 * tree plus metadata like `__layoutFlags` which is a plain object).
 */
...
export function buildOutgoingAppPayload(input: {
  element:
    | TextRenderable
    | Readonly<
        Record<string, TextRenderable | AppElementsInterception | readonly AppElementsSlotBinding[]>
      >
  artifactCompatibility?: ArtifactCompatibilityEnvelope
  cacheEntryReuseProof?: CacheEntryReuseProof
  layoutFlags: LayoutFlags
...
export function readAppElementsMetadata(
  ...
  if (
    ...
    throw new Error('[text] Invalid __interceptionContext in App Router payload')
  }

  const rootLayoutTreePath = elements[APP_ROOT_LAYOUT_KEY]
  if (rootLayoutTreePath === undefined) {
    throw new Error('[text] Missing __rootLayout key in App Router payload')
  }
  if (rootLayoutTreePath !== null && typeof rootLayoutTreePath !== 'string') {
    throw new Error('[text] Invalid __rootLayout in App Router payload: expected string or null')
  }
