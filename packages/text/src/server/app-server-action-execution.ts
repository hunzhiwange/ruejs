import { getAndClearActionRevalidationKind, type ActionRevalidationKind } from '../shims/cache.js'
import type { HeadersAccessPhase } from '../shims/headers.js'
import { type FetchCacheMode, setCurrentFetchCacheMode } from '../shims/fetch-cache.js'
...
import {
  getTextErrorDigest,
  parseTextHttpErrorDigest,
  parseTextRedirectDigest,
} from './text-error-digest.js'
...
type AppServerProgressiveActionDecoder = (body: FormData) => Promise<unknown>
type AppServerActionFormStateDecoder = (
  actionResult: unknown,
  body: FormData,
) => Promise<AppRscFormState | undefined>
...
type AppServerActionRscModel<TElement> = {
  /**
   * Omitted when the action did not invalidate page data. This mirrors Text.js'
   * empty RSC payload for non-revalidating fetch actions: the client resolves
...
type RenderServerActionRscStreamOptions<TActionReferences> = {
  onError: (error: unknown) => unknown
  references: TActionReferences
}

type DecodeServerActionReplyOptions<TActionReferences> = {
  references: TActionReferences
}
...
export type HandleProgressiveServerActionRequestOptions = {
  ...
  /** Configured text.config `basePath`. Prefixed onto progressive Location targets. */
  basePath?: string
  cleanPathname: string
...
> = {
  actionId: string | null
  allowedOrigins: string[]
  /** Configured text.config `basePath`. Prefixed onto ACTION_REDIRECT_HEADER targets. */
  basePath?: string
  buildPageElement: (
  ...
  createActionReferenceSet: () => TActionReferences
  parseActionArgs: (
    body: string | FormData,
    options: DecodeServerActionReplyOptions<TActionReferences>,
  ...
  renderToReadableStream: (
    ...
    options: RenderServerActionRscStreamOptions<TActionReferences>,
...
/**
 * Matches Text.js' server action argument cap to prevent stack overflow in
...
function getActionRedirect(error: unknown): AppServerActionRedirect | null {
  const digest = getTextErrorDigest(error)
  if (!digest) return null

  const redirect = parseTextRedirectDigest(digest)
...
/**
 * Prepend the configured text.config `basePath` to a server-action redirect
 * target before it goes on the wire.
 *
 * `redirect("/foo")` called from a server action mounted at `/base/...` must
 * land the browser at `/base/foo`, mirroring how Text.js threads basePath
 ...
 *
 * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/action-handler.ts
 */
export function applyActionRedirectBasePath(url: string, basePath: string): string {
  ...
  // the path. We must accept hash too because Text.js redirect targets may
  // contain "#anchor".
...
function getActionHttpFallbackStatus(error: unknown): number | null {
  ...
  const httpError = parseTextHttpErrorDigest(digest)
...
function createServerActionErrorResponse(
  ...
  console.error('[text] Server action error:', error)
...
export async function handleProgressiveServerActionRequest(
  ...
  try {
    ...
    const action = await options.decodeProgressiveAction(body)
    ...
    // becomes an absolute Location URL. Mirrors Text.js, which threads
    ...
    getAndClearActionRevalidationKind()
    options.getAndClearPendingCookies()
    console.error('[text] Server action payload parsing error:', error)
    options.reportRequestError(
      normalizeError(error),
      {
        path: options.cleanPathname,
...
>(
  ...
  // middleware rewrites action POSTs. Matches Text.js behavior:
  ...
  try {
    ...
    const actionReferences = options.createActionReferenceSet()
    const args = await options.parseActionArgs(body, { references: actionReferences })
    ...
    if (shouldSkipPageRendering) {
      ...
      const rscStream = await options.renderToReadableStream(
        { returnValue },
        { references: actionReferences, onError: onRenderError },
      )
