...
export type AppPagePageRequest<TModule extends AppPageModule = AppPageModule> = {
  ...
  request: Request
  /** Normalized x-text-mounted-slots header value. */
...
> = {
  ...
  /**
   * Configured text.config `basePath`. Threaded through `resolveAppPageHead`
 ...
 * creates the page server element, and wires it into the nested layout +
 ...
 *
 * Text.js equivalent: the component tree construction in
 * {@link https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/create-component-tree.tsx|create-component-tree.tsx}
 * and the page head resolution in
 * {@link https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/create-metadata.tsx|create-metadata.tsx}.
 */
...
>(options: BuildPageElementsOptions<TModule, TErrorModule>): Promise<AppElements> {
  ...
  if (hasPageModule && !PageComponent) {
    ...
    const noExportRouteId = AppElementsWire.encodeRouteId(route.pattern, interceptionContext)
  ...
  return buildAppPageElements({
    element: PageComponent ? createAppPageServerElement(PageComponent, pageProps) : null,
    // Fall back to text's built-in default global error module so that
    // uncaught client render errors are caught by the route-level
    // <ErrorBoundary> wrapper in app-page-route-wiring.tsx, mirroring
    // Text.js's behavior when the user has not defined app/global-error.tsx.
    globalErrorModule:
    ...
    slotOverrides,
    renderMode,
    elementRoutePath: opts?.interceptionContext ? routePath : route.pattern,
  })
}

function createAppPageServerElement(
  PageComponent: NonNullable<AppPageModule['default']>,
  ...
  const displayName =
    (PageComponent as { displayName?: string; name?: string }).displayName ?? PageComponent.name
  if (displayName) {
