...
import type {
  TextRedirectPayloadThrowerFactory,
  TextRscRenderOptions,
} from './app-render-adapter.js'
...
type AppPageElement = TextRenderable | Readonly<Record<string, TextRenderable>>
type AppPageRenderableElement = TextRenderable | AppOutgoingElements
...
function getCapturedRscDataPromise(
  ...
  if (!capturedRscDataPromise) {
    throw new Error('[text] Expected captured RSC data while regenerating an app page cache entry')
  }
...
async function dispatchAppPageInner<TRoute extends AppPageDispatchRoute>(
          ...
          async () => {
            ...
            const revalidatedHtmlStream = await revalidatedSsrEntry.handleSsr(
              ...
              },
              {
                ...
                ssrPayload: isAppElementsRecord(revalidatedElement)
                  ? (revalidatedElement as AppWireElements)
                  : undefined,
  ...
  return renderAppPageLifecycle({
    ...
    isPrerender: process.env.TEXT_PRERENDER === '1',
    isProduction: options.isProduction,
    isRscRequest: options.isRscRequest,
    isrDebug: options.isrDebug,
    isrHtmlKey: options.isrHtmlKey,
...
/**
 * Builds an RSC payload that encodes a redirect as a serialized error chunk.
 * We render a tiny element that immediately throws an `Error` whose `digest`
 * is the canonical `TEXT_REDIRECT;...` string. `renderToReadableStream`'s
 * `onError` returns that digest, the active payload renderer serializes the
 * error into the stream, and the client's redirect boundary decodes it via
 * Text-compatible redirect helpers.
 *
 * The thrown error's digest matches Text.js's well-known router error format,
 * so neither text's RSC error handler nor Text.js's reporter logs it as a
 * "real" server error.
 */
function buildRscRedirectPayloadStream(digest: string): ReadableStream<Uint8Array> {
  const error = new Error('TEXT_REDIRECT') as Error & { digest: string }
  error.digest = digest

  return renderRuePayloadToReadableStream(error, {
    onError: () => digest,
  })
}
...
async function renderLayoutSpecialError<TRoute extends AppPageDispatchRoute>(
  ...
  return buildAppPageSpecialErrorResponse({
    basePath: options.basePath,
    buildRscRedirectPayloadStream: rscOptions => buildRscRedirectPayloadStream(rscOptions.digest),
    clearRequestContext: options.clearRequestContext,
    getAndClearPendingCookies,
    isEdgeRuntime: options.isEdgeRuntime,
    isRscRequest: options.isRscRequest,
