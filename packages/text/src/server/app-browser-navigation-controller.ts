...
type BrowserNavigationControllerDeps = {
  ...
  syncHistoryStatePreviousTextUrl?: (previousTextUrl: string | null) => void
}
...
type BrowserNavigationController = {
  ...
  attachBrowserRouterState(
    setter: RueStateSetter<AppRouterState | Promise<AppRouterState>>,
   ...
   * commit effect never fires, so the URL update for the in-flight
...
function performHardNavigationWithLoopGuard(
  ...
  if (readHardNavigationLoopGuard() === targetHref && currentHref === targetHref) {
    ...
    console.error(
      `[text] Prevented repeated hard navigation to ${targetHref}; ` +
  ...
  if (!guardPersisted && currentHref === targetHref) {
    console.error(
      `[text] Hard navigation to ${targetHref} requires a reload-loop guard, ` +
...
export function createAppBrowserNavigationController(
  ...
  const syncHistoryStatePreviousTextUrl = deps.syncHistoryStatePreviousTextUrl ?? (() => {})
  ...
  let textNavigationRenderId = 0
  let activeNavigationId = 0
  const pendingNavigationCommits = new Map<number, () => void>()
  const pendingNavigationPrePaintEffects = new Map<number, () => void>()

  let setBrowserRouterState: RueStateSetter<AppRouterState | Promise<AppRouterState>> | null = null
  ...
  function getBrowserRouterStateSetter(): RueStateSetter<AppRouterState | Promise<AppRouterState>> {
    if (!setBrowserRouterState) {
      throw new Error('[text] Browser router state setter is not initialized')
    }
  ...
  function getBrowserRouterState(): AppRouterState {
    if (!browserRouterStateRef) {
      throw new Error('[text] Browser router state is not initialized')
    }
  ...
  function allocateRenderId(): number {
    textNavigationRenderId += 1
    return textNavigationRenderId
  }
  ...
  function beginPendingBrowserRouterState(): PendingBrowserRouterState {
    ...
    if (!resolvePending) {
      throw new Error('[text] Failed to initialize browser router promise')
    }
   ...
   * given renderId. When Rue supersedes a transition update (rapid
  ...
  function dispatchApprovedVisibleCommit(
    ...
    if (pendingRouterState) {
      // The programmatic navigation is already running inside the Rue transition
  ...
  async function renderNavigationPayload(options: {
    ...
    try {
      ...
      if (approvedCommit === null) {
        throw new Error('[text] Commit decision did not approve a visible commit')
      }
      ...
      queuePrePaintNavigationEffect(
        ...
        options.createNavigationCommitEffect({
          ...
          previousTextUrl: approvedCommit.previousTextUrl,
  ...
  async function commitSameUrlNavigatePayload(
    ...
    if (approvedCommit) {
      ...
      if (latestApproval.approvedCommit) {
        ...
        syncHistoryStatePreviousTextUrl(latestApproval.approvedCommit.previousTextUrl)
      } else {
