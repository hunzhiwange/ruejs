...
type AppPageComponent = AppServerComponent<any>
...
type AppPageBoundaryRenderCommonOptions<TModule extends AppPageModule = AppPageModule> = {
  ...
  /** Configured text.config `basePath`, threaded into file-based metadata href emission. */
  basePath?: string
...
type RenderAppPageHttpAccessFallbackOptions<TModule extends AppPageModule = AppPageModule> = {
   ...
   * Mirrors Text.js's `createNotFoundLoaderTree` behavior for `hasGlobalNotFound`.
...
function wrapRenderedBoundaryElement<TModule extends AppPageModule>(
  ...
  return wrapAppPageBoundaryElement({
    ...
    renderErrorBoundary(GlobalErrorComponent, children) {
      return createAppServerElement(ErrorBoundary, {
    ...
    renderLayout(LayoutComponent, children, asyncParams) {
      return createAppServerElement(LayoutComponent as AppPageComponent, {
        // oxlint-disable-text-line rue/no-children-prop
        children,
        params: asyncParams,
      })
    ...
    renderLayoutSegmentProvider(segmentMap, children) {
      return createAppServerElement(
        // oxlint-disable-text-line @typescript-eslint/no-explicit-any
        LayoutSegmentProvider as AppServerComponent<any>,
        { segmentMap },
        children,
      )
    },
...
export async function renderAppPageHttpAccessFallback<TModule extends AppPageModule>(
  ...
  headElements.push(createAppServerElement(ViewportHead, { key: 'viewport', viewport }))

  const skipLayoutWrapping = options.skipLayoutWrapping ?? false
  const element = wrapRenderedBoundaryElement({
    element: createAppServerElement(
      AppServerFragment,
      null,
      ...headElements,
      createAppServerElement(boundaryComponent),
    ),
...
export async function renderAppPageErrorBoundary<TModule extends AppPageModule>(
  ...
  if (!errorBoundary.isGlobalError) {
    try {
      ...
      if (metadata) {
        headElements.push(
          createAppServerElement(MetadataHead, { key: 'metadata', metadata, pathname }),
        )
      }
      headElements.push(createAppServerElement(ViewportHead, { key: 'viewport', viewport }))
    } catch (error) {
      console.error(
        `[text] App page error boundary head resolution failed for ${options.route?.pattern ?? pathname}:`,
        error,
      )
    }
  }
  ...
  const element = wrapRenderedBoundaryElement({
    element: createAppServerElement(
      ...
      null,
      ...headElements,
      createAppServerElement(errorBoundary.component, {
        error: errorObject,
      }),
    ),
    globalErrorModule: options.globalErrorModule,
...
// Rue client-only hooks that are absent from the `rue-server` export
...
// the format used by the text/navigation shims (see client-hook-error.ts).
