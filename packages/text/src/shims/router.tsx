import { _getSSRContext } from './router-context-state.js'
export { setSSRContext, _registerRouterStateAccessors } from './router-context-state.js'
/**
 * text/router shim
 *
...
function PagesRouterProvider({ children }: { children: RueNode }): RueElement {
  const [routeState, setState] = useState(getPathnameAndQuery)
  ...
  return (
    <RouterContext.Provider
      value={buildRouterValue(routeState.pathname, routeState.query, routeState.asPath, {
        push: Router.push,
        replace: Router.replace,
        back: Router.back,
        reload: Router.reload,
...
/**
 * Wrap a compiled plan in a RouterContext.Provider so that
 ...
 */
export function wrapWithRouterContext(plan: any): any {
  return PagesRouterProvider({ children: plan })
}
...
export function withRouter<P extends WithRouterProps>(
  ...
  function WithRouterWrapper(props: ExcludeRouterProps<P>): RueElement {
    ...
    return <ComposedComponent {...(merged as unknown as P)} />
  }
